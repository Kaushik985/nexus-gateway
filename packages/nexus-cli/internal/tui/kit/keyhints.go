...
func HelpReference() string {
	...
	b.WriteString("- `/<view>` — open a view: overview, radar, cost, slo, nodes, alerts, compliance, jobs, sync, models, keys, rules, kill, lab, chat\n")
	...
	b.WriteString("- `/event <id>` — open a traffic event · `/sessions` — resume a past conversation\n")
	b.WriteString("- `/clear` — reset the conversation · `/compact` — condense older turns · `/context` — context breakdown\n")
	b.WriteString("- `/verbose` — expand tool I/O · `/env` — switch environment · `/login` · `/logout`\n\n")
