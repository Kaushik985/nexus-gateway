// Tests for the Azure OpenAI adapter transport + spec wiring. The adapter is
// thin (deployment-name path + api-version query + api-key header); the
// underlying codec, error normalizer and stream decoder are reused from
// openai. These tests pin every Azure-specific divergence and the Probe
// contract:
//
