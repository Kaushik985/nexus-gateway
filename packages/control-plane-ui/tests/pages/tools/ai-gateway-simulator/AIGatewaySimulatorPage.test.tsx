...
import { AIGatewaySimulatorPage } from '../../../../src/pages/tools/ai-gateway-simulator/AIGatewaySimulatorPage';
...
describe('AIGatewaySimulatorPage', () => {
  ...
  it('Clear wipes the conversation after a reply', async () => {
    renderWithProviders(<AIGatewaySimulatorPage />);
    await openSettingsAndConnect();

    await userEvent.type(screen.getByLabelText(/chat input/i), 'hello');
    await userEvent.click(screen.getByRole('button', { name: /^send$/i }));
    await screen.findByText(/hello from assistant/i);

    // Clear is enabled once there are messages; clicking it empties the timeline
    await userEvent.click(screen.getByRole('button', { name: /^clear$/i }));
    expect(screen.queryByText(/hello from assistant/i)).toBeNull();
  });
