...
func TestCodec_DecodeResponse_UnsupportedEndpoint(t *testing.T) {
	_, err := codec{}.DecodeResponse(typology.WireShapeOpenAIEmbeddings, []byte(`{}`), "")
	if err == nil || !strings.Contains(err.Error(), "unsupported endpoint") {
		t.Fatalf("expected unsupported-endpoint error, got %v", err)
	}
}
...
func TestCodec_DecodeResponse_EmptyBody(t *testing.T) {
	decRes, err := codec{}.DecodeResponse(typology.WireShapeOpenAIChat, nil, "")
...
func TestCodec_DecodeResponse_InvalidJSON(t *testing.T) {
	_, err := codec{}.DecodeResponse(typology.WireShapeOpenAIChat, []byte(`{not json`), "")
...
// `event: error` branch: the chunk must carry the error text in Delta,
// flip Done=true, and any subsequent Next call must return io.EOF.
func TestStreamSession_Next_ErrorEventTerminatesStream(t *testing.T) {
	...
	if c.Delta != "model crashed" {
		t.Errorf("Delta=%q want 'model crashed'", c.Delta)
	}
	if !c.Done {
		t.Errorf("error event must flip Done=true, got %+v", c)
	}
	if c.NativeEvent != "error" {
		t.Errorf("NativeEvent=%q want 'error'", c.NativeEvent)
	}
