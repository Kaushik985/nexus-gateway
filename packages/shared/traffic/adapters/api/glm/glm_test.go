...
func TestExtractRequest_OpenAICompatDelegation(t *testing.T) {
	body := []byte(`{
		"model":"glm-4",
		"messages":[{"role":"user","content":"hi from glm"}]
	}`)
	a := &Adapter{}
	nc, err := a.ExtractRequest(context.Background(), body, "/api/paas/v4/chat/completions")
	if err != nil {
		t.Fatalf("err=%v", err)
	}
	if len(nc.Segments) != 1 || nc.Segments[0] != "hi from glm" {
...
func TestDetectRequestMeta(t *testing.T) {
	...
	r, _ := http.NewRequest(http.MethodPost, "https://open.bigmodel.cn/api/paas/v4/chat/completions", nil)
	// GLM JWT (shape: header.payload.signature, opaque base64url)
	r.Header.Set("Authorization", "Bearer eyJhbGciOiJIUzI1NiJ9.glm.sig")
...
func TestDetectResponseUsage(t *testing.T) {
	a := &Adapter{}
	body := []byte(`{"usage":{"prompt_tokens":20,"completion_tokens":80}}`)
	um := a.DetectResponseUsage(nil, body)
	if um.Status != traffic.UsageStatusOK {
		t.Fatalf("status = %q", um.Status)
	}
	if *um.PromptTokens != 20 || *um.CompletionTokens != 80 {
		t.Errorf("tokens = %d/%d", *um.PromptTokens, *um.CompletionTokens)
	}
}

// Configure is a no-op for glm (delegates to openai inner which is also
...
// surface end-to-end through the glm wrapper.
func TestExtractResponse_ChatCompletionsDelegation(t *testing.T) {
	...
	a := &Adapter{}
	nc, err := a.ExtractResponse(context.Background(), body, "/api/paas/v4/chat/completions")
	if err != nil {
		t.Fatalf("err=%v", err)
	}
	if len(nc.Segments) != 1 || nc.Segments[0] != "hello from glm" {
		t.Errorf("Segments=%v", nc.Segments)
	}
...
func TestExtractResponse_Malformed(t *testing.T) {
	a := &Adapter{}
	_, err := a.ExtractResponse(context.Background(), []byte(`not json`), "/api/paas/v4/chat/completions")
...
// delta must produce a single Segment. The glm wrapper does no
// transform here.
func TestExtractStreamChunk_ContentDelta(t *testing.T) {
	chunk := []byte(`{"choices":[{"index":0,"delta":{"content":"hi"}}]}`)
	a := &Adapter{}
	nc, err := a.ExtractStreamChunk(context.Background(), chunk, "/api/paas/v4/chat/completions")
	if err != nil {
		t.Fatalf("err=%v", err)
	}
...
func TestExtractStreamChunk_Malformed(t *testing.T) {
	a := &Adapter{}
	_, err := a.ExtractStreamChunk(context.Background(), []byte(`not json`), "/api/paas/v4/chat/completions")
	if !errors.Is(err, traffic.ErrMalformed) {
		t.Errorf("err=%v want ErrMalformed", err)
	}
}
...
// glm wrapper preserves the rewritten output and patch count.
func TestRewriteResponseBody_ChatCompletionsDelegation(t *testing.T) {
	...
	a := &Adapter{}
	out, n, err := a.RewriteResponseBody(context.Background(), body, "/api/paas/v4/chat/completions", traffic.NormalizedContent{
...
// confirms the wrapper does not invent a stale "glm-jwt" tag.
