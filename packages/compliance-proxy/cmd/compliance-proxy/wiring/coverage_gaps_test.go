...
// the Health.Run closure (lines 65-80) by hitting /runtime/health.
// COMPLIANCE_PROXY_API_TOKEN is not set in tests so auth is in dev-mode
// (disabled), meaning no bearer token is needed.
func TestInitRuntimeAPIServer_HealthRunEndpoint_InvokesHealthRunClosure(t *testing.T) {
	...
	// GET /runtime/health — invokes Health.Run closure (auth disabled in dev mode).
	resp, getErr := http.Get("http://" + addr + "/runtime/health")
	if getErr != nil {
