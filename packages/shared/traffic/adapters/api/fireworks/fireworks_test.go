...
func TestExtractRequest_OpenAICompatDelegation(t *testing.T) {
	body := []byte(`{
		...
		"messages":[{"role":"user","content":"hi from fireworks"}]
	}`)
	a := &Adapter{}
	...
	if len(nc.Segments) != 1 || nc.Segments[0] != "hi from fireworks" {
...
func TestDetectRequestMeta_FireworksKeyClass(t *testing.T) {
	body := []byte(`{"messages":[{"role":"user","content":"hi"}]}`)
	a := &Adapter{}
	r, _ := http.NewRequest(http.MethodPost, "https://api.fireworks.ai/v1/chat/completions", nil)
	r.Header.Set("Authorization", "Bearer fw_xxx")
	meta := a.DetectRequestMeta(r, body)
	if meta.Provider != "fireworks" {
		t.Errorf("Provider=%q", meta.Provider)
	}
	if meta.ApiKeyClass != "fireworks-bearer" {
		t.Errorf("ApiKeyClass=%q", meta.ApiKeyClass)
	}
}

// Configure is a no-op for fireworks (delegates to openai inner which is also
...
// surface end-to-end through the fireworks wrapper.
func TestExtractResponse_ChatCompletionsDelegation(t *testing.T) {
	...
	if len(nc.Segments) != 1 || nc.Segments[0] != "hello from fireworks" {
		t.Errorf("Segments=%v", nc.Segments)
	}
...
// delta must produce a single Segment. The fireworks wrapper does no
// transform here.
...
// rewriter and substitutes per-message text — pin that the fireworks
...
// fireworks wrapper preserves the rewritten output and patch count.
...
// confirms the wrapper does not invent a stale "fireworks-bearer" tag.
func TestDetectRequestMeta_NoAuth(t *testing.T) {
	body := []byte(`{"model":"accounts/fireworks/models/llama-v3p3-70b-instruct","messages":[{"role":"user","content":"hi"}]}`)
	a := &Adapter{}
	r, _ := http.NewRequest(http.MethodPost, "https://api.fireworks.ai/v1/chat/completions", nil)
	meta := a.DetectRequestMeta(r, body)
	if meta.ApiKeyClass != "" {
	...
	if meta.Model != "accounts/fireworks/models/llama-v3p3-70b-instruct" {
		t.Errorf("Model=%q", meta.Model)
	}
}

// DetectRequestMeta with a non-Bearer Authorization scheme: the wrapper
// must NOT stamp "fireworks-bearer" — non-Bearer headers are not API
...
func TestDetectRequestMeta_NilRequest(t *testing.T) {
	body := []byte(`{"model":"accounts/fireworks/models/llama-v3p3-70b-instruct"}`)
	a := &Adapter{}
...
// fireworks wrapper passes through unchanged, so prompt/completion split
...
// Normalize: fireworks speaks the openai-chat wire shape. A canonical
// chat-completions body must claim Tier-1 with DetectedSpec=fireworks
...
func TestNormalize_OpenAIChatShape(t *testing.T) {
	...
	if payload.Model != "accounts/fireworks/models/llama-v3p3-70b-instruct" {
