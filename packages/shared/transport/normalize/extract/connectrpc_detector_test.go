...
//
// Goal: the existing accumulator_test / detector_test /
// normalizer_test / probe_test / sse_test files cover happy paths and
// the load-bearing prod regressions; this file pins the error/branch
// arms (parsePointer / setAtPointer / removeAtPointer / mapRole /
// public spec lookups / Tier-1 adapter dispatch / detector fallback
// paths).
//
...
// TestConnectRPCProtobufDetector_Decode_ResponseGzipFrame: a Connect-RPC
// response frame flagged compressed (0x01) carries a gzip-compressed
// StreamChatResponse. The detector must gunzip the frame before extracting the
// text delta — feeding the compressed bytes straight to the protobuf parser
// produced a stray "j" before this fix.
func TestConnectRPCProtobufDetector_Decode_ResponseGzipFrame(t *testing.T) {
	// StreamChatResponse: field 1 = "compressed reply".
	var inner []byte
	inner = protowire.AppendTag(inner, 1, protowire.BytesType)
	inner = protowire.AppendString(inner, "compressed reply")
	...
	// Frame flagged 0x01 (compressed), followed by a 0x02 trailer.
	frame := make([]byte, 5)
	frame[0] = 0x01
	binary.BigEndian.PutUint32(frame[1:], uint32(gz.Len()))
	frame = append(frame, gz.Bytes()...)
	frame = append(frame, buildConnectFrame(nil, true)...)

	det, ok := (ConnectRPCProtobufDetector{}).Decode(frame, "response")
	if !ok {
		t.Fatal("Decode(gzip frame) ok=false, want true")
	}
	if det.AssistantText != "compressed reply" {
		t.Errorf("assistantText = %q, want 'compressed reply'", det.AssistantText)
	}
}
...
func TestChatSpecByID_Known(t *testing.T) {
	s := ChatSpecByID("chatgpt-web")
	if s == nil || s.ID != "chatgpt-web" || s.Locator != "messages" {
		t.Fatalf("got %+v", s)
	}
}
...
func TestChatResponseSpecByID(t *testing.T) {
	if s := ChatResponseSpecByID("chatgpt-web"); s == nil || s.ID != "chatgpt-web" {
...
func TestBuildPayload_PerAdapterFlavor(t *testing.T) {
	...
	if out.Protocol != "claude-web" {
		t.Errorf("Protocol: %q want claude-web", out.Protocol)
	}
	if out.DetectedSpec != "claude-web" {
...
func TestNormalizeForAdapter_RequestClaimsPerSpec(t *testing.T) {
	...
	if out.DetectedSpec != "chatgpt-web" {
		t.Errorf("DetectedSpec: %q want chatgpt-web (no pattern: prefix)", out.DetectedSpec)
	}
	if out.Kind != normalize.KindAIChat {
		t.Errorf("Kind: %v", out.Kind)
	}
...
// TestScoreResponseSpec_SSESpecVsNonSSEBody: the response probe only
// recognises SSE framing — a single-JSON body returns a non-stream
// zero-detection (mismatch path).
...
// TestScoreChatSpec_SingleStringPromptVariant: the flat-prompt legacy
// spec (Locator empty, ContentPath="prompt") gives a confidence bump
// when the body has a non-empty prompt string.
func TestScoreChatSpec_SingleStringPromptVariant(t *testing.T) {
	spec := *ChatSpecByID("openai-completions-legacy")
...
// TestScoreResponseSpec_FullCoverageAndShorthandFrames: shorthand
// continuation frames (no p/o, just v) count as patch candidates and
// apply via the last-append path; a fully-applied stream with a
// signature hit scores coverage 1.0.
func TestScoreResponseSpec_FullCoverageAndShorthandFrames(t *testing.T) {
	spec := *ChatResponseSpecByID("chatgpt-web")
	...
	if !d.IsStream || d.Confidence != 1.0 {
		t.Errorf("conf: %v stream: %v (want coverage 1.0)", d.Confidence, d.IsStream)
	}
	if d.AssistantText != "ABC" {
		t.Errorf("text: %q want ABC", d.AssistantText)
	}
}
