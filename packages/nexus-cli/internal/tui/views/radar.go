...
func newRadar(gw kit.Gateway) *Radar { return &Radar{gw: gw, loading: true} }
...
func (r *Radar) Update(msg tea.Msg) (kit.ViewModel, tea.Cmd) {
	switch msg := msg.(type) {
	case radarMsg:
		r.loading = false
		r.err = msg.err
		...
		r.clampCursor()
		return r, kit.Tick(kit.PollFast, radarTick{})
	case radarTick:
		return r, r.fetch()
	case tea.KeyPressMsg:
		...
		case "enter":
			if id := r.selectedID(); id != "" {
				return r, func() tea.Msg { return kit.OpenEventMsg{ID: id} }
			}
		}
	}
	return r, nil
...
// ErrorsOnly reports whether the agent's errors-only filter drive is active.
func (r *Radar) ErrorsOnly() bool { return r.errorsOnly }

// Filter returns the active traffic filter (set by the agent's navigate drive).
func (r *Radar) Filter() core.TrafficFilter { return r.base }
