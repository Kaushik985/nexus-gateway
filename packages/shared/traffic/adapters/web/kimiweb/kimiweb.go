// Package kimiweb implements the kimi-web traffic adapter for
// browser-side traffic to kimi.com and kimi.moonshot.cn — Moonshot's
// consumer Kimi chat product. Distinct from api.moonshot.cn /
// api.moonshot.ai (handled by the moonshot adapter).
//
...
func (a *Adapter) DetectRequestMeta(_ *http.Request, body []byte) traffic.RequestMeta {
	meta := traffic.RequestMeta{Provider: "kimi-web"}
	if !gjson.ValidBytes(body) {
		return meta
	}
