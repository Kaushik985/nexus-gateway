...
func TestDecodeResponse_EmptyBody(t *testing.T) {
	decRes, err := codec{}.DecodeResponse(typology.WireShapeGeminiGenerateContent, nil, "")
	out := decRes.CanonicalBody
	if err != nil {
		t.Fatal(err)
	}
...
func TestDecodeResponse_MalformedNativeBody(t *testing.T) {
	decRes, err := codec{}.DecodeResponse(typology.WireShapeGeminiGenerateContent, []byte(`{"candidates":"not an array"}`), "")
	out := decRes.CanonicalBody
	if err != nil {
		t.Fatal(err)
	}
