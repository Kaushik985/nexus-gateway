...
func TestRegisterRoutesMountsSplit(t *testing.T) {
	e := echo.New()
	New(Config{}).RegisterAssistantRoutes(e.Group("/api/admin"), nil)
...
func TestChatStreamToolRoundStreamsActivity(t *testing.T) {
	...
	// The tool's result rides on tool_end so the widget chip can show the
	// RESPONSE, not just the request (the observe_health tool errors against
	// the {} mock backend — what matters is the output field is populated).
	if !strings.Contains(out, `"output":`) {
		t.Fatalf("tool_end must carry the tool output peek, got:\n%s", out)
	}
...
// TestClampToolOutput bounds the tool_end payload: small outputs pass through
// byte-exact, oversized ones truncate on a rune boundary with an honest marker
// naming the full size.
func TestClampToolOutput(t *testing.T) {
	if got := clampToolOutput([]byte(`{"ok":true}`)); got != `{"ok":true}` {
		t.Fatalf("small output must pass through, got %q", got)
	}
	big := strings.Repeat("界", toolOutputCap) // 3 bytes per rune → well past the cap
	got := clampToolOutput([]byte(big))
	if len(got) > toolOutputCap+64 {
		t.Fatalf("clamped output too large: %d bytes", len(got))
	}
	if !strings.Contains(got, fmt.Sprintf("truncated — %d bytes total", len(big))) {
		t.Fatalf("marker must name the full size, got tail %q", got[len(got)-60:])
	}
	if !utf8.ValidString(got) {
		t.Fatal("truncation must not split a rune")
	}
	// Binary (non-UTF-8) output keeps the full capped prefix — the rune
	// walk-back is bounded and must never collapse the peek to the first
	// invalid byte (review F5: a binary fs_read result).
	bin := bytes.Repeat([]byte{0x80}, toolOutputCap+64) // continuation bytes — never a rune start
	got = clampToolOutput(bin)
	if len(got) < toolOutputCap {
		t.Fatalf("binary output peek collapsed to %d bytes; want the full %d-byte prefix", len(got), toolOutputCap)
	}
}
