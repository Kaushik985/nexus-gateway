...
// DimensionColumnMap + PrimaryDimensions — load-bearing constants used by
// rollup SQL generation; assert presence and column-name pairing so renames
// surface here, not in a silent prod SQL miss.

func TestDimensionColumnMapPairing(t *testing.T) {
	...
	if len(DimensionColumnMap) != len(want) {
		t.Errorf("DimensionColumnMap size = %d, want %d", len(DimensionColumnMap), len(want))
	}
	for k, wantCol := range want {
		gotCol, ok := DimensionColumnMap[k]
		if !ok {
			t.Errorf("DimensionColumnMap[%q] missing", k)
			continue
		}
		if gotCol != wantCol {
			t.Errorf("DimensionColumnMap[%q] = %q, want %q", k, gotCol, wantCol)
		}
	}

	// PrimaryDimensions must be exactly the keys of DimensionColumnMap.
	if len(PrimaryDimensions) != len(DimensionColumnMap) {
		t.Errorf("PrimaryDimensions len = %d, want %d", len(PrimaryDimensions), len(DimensionColumnMap))
	}
	seen := make(map[string]bool, len(PrimaryDimensions))
	for _, d := range PrimaryDimensions {
		if _, ok := DimensionColumnMap[d]; !ok {
			t.Errorf("PrimaryDimensions contains %q with no DimensionColumnMap entry", d)
		}
		seen[d] = true
	}
	for k := range DimensionColumnMap {
		if !seen[k] {
			t.Errorf("DimensionColumnMap key %q absent from PrimaryDimensions", k)
		}
	}
}
