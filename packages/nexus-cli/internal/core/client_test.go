...
func TestClient_GetJSON(t *testing.T) {
	c, done := testClient(t, http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		if r.URL.Path != "/api/admin/analytics/cost" {
			t.Errorf("path %s", r.URL.Path)
		}
		_, _ = io.WriteString(w, `{"total":12.5}`)
	}))
	defer done()
	var out struct {
		Total float64 `json:"total"`
	}
	if err := c.GetJSON(context.Background(), "/api/admin/analytics/cost", nil, &out); err != nil {
		t.Fatalf("GetJSON: %v", err)
	}
	if out.Total != 12.5 {
		t.Fatalf("GetJSON decoded %v", out.Total)
	}
}
...
func TestMetricsAggregates(t *testing.T) {
	c, done := testClient(t, http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		if r.URL.Path != "/api/admin/metrics/aggregates" {
			t.Errorf("path %s", r.URL.Path)
		}
		_, _ = io.WriteString(w, `{"summary":{"totalTokens":99}}`)
	}))
	defer done()
	got, err := c.MetricsAggregates(context.Background(), nil)
	if err != nil || got.Summary["totalTokens"] != 99 {
		t.Fatalf("MetricsAggregates wrong: %+v err=%v", got, err)
	}
}
