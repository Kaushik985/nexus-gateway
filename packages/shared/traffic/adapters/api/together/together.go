// Package together implements the together traffic adapter for
...
// DetectRequestMeta delegates then re-labels Provider as "together".
// Together AI tokens are typically opaque hex strings; we classify
// them under the "together-bearer" key class.
func (a *Adapter) DetectRequestMeta(r *http.Request, body []byte) traffic.RequestMeta {
	meta := a.inner.DetectRequestMeta(r, body)
	meta.Provider = "together"
	if r != nil {
		auth := r.Header.Get("Authorization")
		if strings.HasPrefix(auth, "Bearer ") {
			tok := strings.TrimSpace(auth[len("Bearer "):])
			if tok != "" {
				meta.ApiKeyClass = "together-bearer"
