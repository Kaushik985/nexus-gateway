...
func TestExtractRequest_OpenAICompatDelegation(t *testing.T) {
	body := []byte(`{
		...
		"messages":[{"role":"user","content":"hi from together"}]
	}`)
	a := &Adapter{}
	...
	if len(nc.Segments) != 1 || nc.Segments[0] != "hi from together" {
...
func TestDetectRequestMeta_TogetherKeyClass(t *testing.T) {
	body := []byte(`{"messages":[{"role":"user","content":"hi"}]}`)
	a := &Adapter{}
	r, _ := http.NewRequest(http.MethodPost, "https://api.together.xyz/v1/chat/completions", nil)
	r.Header.Set("Authorization", "Bearer together_api_key_abc")
	meta := a.DetectRequestMeta(r, body)
	if meta.Provider != "together" {
		t.Errorf("Provider=%q", meta.Provider)
	}
	if meta.ApiKeyClass != "together-bearer" {
		t.Errorf("ApiKeyClass=%q", meta.ApiKeyClass)
	}
}

// Configure is a no-op for together (delegates to openai inner which is also
...
// surface end-to-end through the together wrapper.
func TestExtractResponse_ChatCompletionsDelegation(t *testing.T) {
	...
	if len(nc.Segments) != 1 || nc.Segments[0] != "hello from together" {
		t.Errorf("Segments=%v", nc.Segments)
	}
...
// delta must produce a single Segment. The together wrapper does no
// transform here.
...
// rewriter and substitutes per-message text — pin that the together
...
// together wrapper preserves the rewritten output and patch count.
...
// confirms the wrapper does not invent a stale "together-bearer" tag.
func TestDetectRequestMeta_NoAuth(t *testing.T) {
	body := []byte(`{"model":"meta-llama/Llama-3.3-70B-Instruct-Turbo","messages":[{"role":"user","content":"hi"}]}`)
	a := &Adapter{}
	r, _ := http.NewRequest(http.MethodPost, "https://api.together.xyz/v1/chat/completions", nil)
	meta := a.DetectRequestMeta(r, body)
	if meta.ApiKeyClass != "" {
	...
	if meta.Model != "meta-llama/Llama-3.3-70B-Instruct-Turbo" {
		t.Errorf("Model=%q", meta.Model)
	}
}

// DetectRequestMeta with a non-Bearer Authorization scheme: the wrapper
// must NOT stamp "together-bearer" — non-Bearer headers are not API
...
func TestDetectRequestMeta_NilRequest(t *testing.T) {
	body := []byte(`{"model":"meta-llama/Llama-3.3-70B-Instruct-Turbo"}`)
	a := &Adapter{}
...
// together wrapper passes through unchanged, so prompt/completion split
...
// Normalize: together speaks the openai-chat wire shape. A canonical
// chat-completions body must claim Tier-1 with DetectedSpec=together
// and surface the user prompt + model on the normalized payload.
func TestNormalize_OpenAIChatShape(t *testing.T) {
	body := []byte(`{
		"model":"meta-llama/Llama-3.3-70B-Instruct-Turbo",
		"messages":[{"role":"user","content":"hello together"}]
	}`)
	a := &Adapter{}
	...
	if payload.Model != "meta-llama/Llama-3.3-70B-Instruct-Turbo" {
