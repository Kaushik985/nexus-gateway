// Tests for proxy handler helpers that have no DB / executor / upstream
// dependencies: ingress context helpers, error/envelope helpers, request
// body parsing, retry-policy parsing, and the small scalar/streaming
// helpers — exercised without standing up a full proxy pipeline.
package proxy
...
type keyCapturingLimiter struct {
	keys    []string
	exhaust map[string]bool
}

func (k *keyCapturingLimiter) Allow(key string, _ int, _ int64) (bool, int) {
...
func (k *keyCapturingLimiter) sawKey(want string) bool {
...
func TestCheckRateLimit_KeyedOnVKID_NotName(t *testing.T) {
	rpm := 100
	lim := &keyCapturingLimiter{exhaust: map[string]bool{"vk-A": true}}
	h := &Handler{deps: &Deps{RateLimiter: lim}}

	a := &vkauth.VKMeta{ID: "vk-A", Name: "shared-label", RateLimitRpm: &rpm}
	b := &vkauth.VKMeta{ID: "vk-B", Name: "shared-label", RateLimitRpm: &rpm}

	if err := h.checkRateLimit(httptest.NewRecorder(), a); err == nil {
		t.Error("VK A's exhausted bucket should rate-limit A")
	}
	if err := h.checkRateLimit(httptest.NewRecorder(), b); err != nil {
		t.Errorf("VK B (same Name, distinct ID) must NOT share A's bucket: %v", err)
	}

	if lim.sawKey("shared-label") {
		t.Error("limiter was keyed on the non-unique Name 'shared-label'")
	}
	if !lim.sawKey("vk-A") || !lim.sawKey("vk-B") {
		t.Errorf("expected per-ID keys vk-A and vk-B; got %v", lim.keys)
	}
}
...
func TestCheckCompareRateLimit_KeyedOnVKID(t *testing.T) {
	...
	h := &Handler{deps: &Deps{RateLimiter: lim}}
	if err := h.checkCompareRateLimit(httptest.NewRecorder(), &vkauth.VKMeta{ID: "vk-X", Name: "n"}); err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if !lim.sawKey("vk-X:compare") {
		t.Errorf("compare bucket not keyed on '<id>:compare'; got %v", lim.keys)
	}
	if lim.sawKey("n:compare") {
		t.Error("compare bucket keyed on the non-unique Name")
	}
}
...
type fakeCachePricing struct {
	row *store.CachePricing
}

func (f *fakeCachePricing) LookupCachePricing(_ string) *store.CachePricing {
	return f.row
}
...
func TestProxy_ComputeCacheCosts_NoCacheTokens(t *testing.T) {
	h := &Handler{deps: &Deps{CachePricing: &fakeCachePricing{row: &store.CachePricing{}}}}
...
// sequence of chunks for chunkSSEReader tests. closeErr, when set, is
// returned from Close so teardown-failure tolerance can be asserted.
...
func TestUsageSummaryHandler_AuthMissing(t *testing.T) {
	...
	authn := vkauth.NewAuthenticator(&fakeVKLookup{}, vkKeyring("test-secret"), slog.Default())
