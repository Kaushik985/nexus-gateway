...
func TestExtractRequest_Delegation(t *testing.T) {
	body := []byte(`{
	...
	if len(nc.Segments) != 1 || nc.Segments[0] != "hello kimi" {
...
func TestDetectRequestMeta(t *testing.T) {
	...
	a := &Adapter{}
	r, _ := http.NewRequest(http.MethodPost, "https://api.moonshot.ai/v1/chat/completions", nil)
	r.Header.Set("Authorization", "Bearer sk-moonshot-xyz")
	meta := a.DetectRequestMeta(r, body)
	if meta.Provider != "moonshot" {
		t.Errorf("Provider=%q", meta.Provider)
	}
	if meta.ApiKeyClass != "moonshot-bearer" {
		t.Errorf("ApiKeyClass=%q", meta.ApiKeyClass)
	}
}

// Configure is a no-op for moonshot (delegates to openai inner which is also
...
// surface end-to-end through the moonshot wrapper.
func TestExtractResponse_ChatCompletionsDelegation(t *testing.T) {
	...
	if len(nc.Segments) != 1 || nc.Segments[0] != "hello from moonshot" {
		t.Errorf("Segments=%v", nc.Segments)
	}
...
// delta must produce a single Segment. The moonshot wrapper does no
// transform here.
...
// rewriter and substitutes per-message text — pin that the moonshot
...
// moonshot wrapper preserves the rewritten output and patch count.
...
// confirms the wrapper does not invent a stale "moonshot-bearer" tag.
func TestDetectRequestMeta_NoAuth(t *testing.T) {
	body := []byte(`{"model":"moonshot-v1-8k","messages":[{"role":"user","content":"hi"}]}`)
	a := &Adapter{}
	r, _ := http.NewRequest(http.MethodPost, "https://api.moonshot.ai/v1/chat/completions", nil)
	meta := a.DetectRequestMeta(r, body)
	if meta.ApiKeyClass != "" {
...
func TestDetectRequestMeta_NilRequest(t *testing.T) {
	body := []byte(`{"model":"moonshot-v1-8k"}`)
	a := &Adapter{}
...
// moonshot wrapper passes through unchanged, so prompt/completion split
...
// Normalize: moonshot speaks the openai-chat wire shape. A canonical
// chat-completions body must claim Tier-1 with DetectedSpec=moonshot
...
func TestNormalize_OpenAIChatShape(t *testing.T) {
	...
	if payload.Model != "moonshot-v1-8k" {
