...
func (v *labView) SetSession(s kit.Session) { v.session = s }

func newLab(gw kit.Gateway, s kit.Session) *labView {
	ta := textarea.New()
	ta.SetHeight(8)
	ta.SetWidth(60)
...
func (l *labView) Help() string {
	if l.editing {
		return "esc exit edit · ctrl+s send lab request · tab chat"
	}
	return "g generator burst · e edit body · r send lab · t route dry-run · q quit"
}

func (l *labView) Update(msg tea.Msg) (kit.ViewModel, tea.Cmd) {
	switch msg := msg.(type) {
	case genResultMsg:
		if msg.ok {
			l.genOK++
	...
	case labResultMsg:
		l.labBusy = false
		l.labErr = msg.err
		l.labResp = kit.PrettyJSON(msg.body)
		return l, nil
	case routeResultMsg:
		l.routeBusy = false
		l.routeErr = msg.err
	...
	case tea.PasteMsg:
		// Pasted text edits the request body, but only while the editor is focused.
		if l.editing {
			var cmd tea.Cmd
...
func (l *labView) updateEditing(msg tea.KeyPressMsg) (kit.ViewModel, tea.Cmd) {
	switch msg.String() {
	case "esc":
		l.editing = false
		l.editor.Blur()
...
func (l *labView) routePanel() string {
	...
	default:
		b.WriteString(styles.TileLabel.Render("press t to resolve the route for " + kit.Dash(l.session.Model)))
	}
	return b.String()
}
...
func (l *labView) genStatus() string {
	if !l.genRunning && l.genTotal == 0 {
		return styles.TileLabel.Render(fmt.Sprintf("press g to fire %d-request burst against %s", generatorBurstSize, kit.Dash(l.session.Model)))
	}
	state := "DONE"
	color := styles.Green
	if l.genRunning {
