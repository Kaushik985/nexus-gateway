// packages/ai-gateway/internal/policy/aiguard/coverage_test.go
//
// Tests for the aiguard package coverage. Each test below asserts
// observable behavior (decision, error shape, side-effects, registered
// collectors) rather than padding invocation counts.
package aiguard
...
func TestExternalBackend_NilHTTPClient(t *testing.T) {
	b := &ExternalBackend{URL: "http://example.com", APIKey: "k", Model: "m"} // HTTPClient nil
...
func (nilRespAdapter) PrepareBody(req provcore.Request) ([]byte, []string, error) {
	return req.Body, nil, nil
}
func (nilRespAdapter) ExecuteWithBody(ctx context.Context, req provcore.Request, body []byte, _ []string) (*provcore.Response, error) {
	req.Body = body
