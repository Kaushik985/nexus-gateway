// Tests for the wiring subsystem constructors: InitDB / NewResolver / VKAuth /
// Executor / Redis / AuditWriter / Router / GeminiCacheMgrSet init paths and
// the LiveClassifier backend builder.
package wiring
...
func TestInitDB_invalidDSNReturnsError(t *testing.T) {
	db, err := InitDB(context.Background(), config.DatabaseConfig{URL: "postgres://invalid:5432/nonexistent_db_xyz?connect_timeout=1"})
	if err == nil {
...
func TestInitVKAuth_withNonNilCacheLayer(t *testing.T) {
	_, l := newLayerWithMock(t)
	auth, err := InitVKAuth(l, cfgWithHMAC("valid-hmac-secret-key"), discardLogger())
	if err != nil {
...
func TestMountAIGuardRoutes_complianceWebhookRegistered(t *testing.T) {
	...
	if rrNoToken.Code == http.StatusNotFound {
		t.Fatalf("expected /v1/ai-guard/compliance-webhook to be registered, got 404")
	}
	if rrNoToken.Code != http.StatusUnauthorized {
		t.Errorf("compliance-webhook without token: got %d, want 401 (route must be auth-gated)", rrNoToken.Code)
	}

	// With the correct token the request clears the auth gate (i.e. it does NOT 401);
	// it may then fail downstream on the nil-dependency body, but auth has passed.
	reqWithToken := httptest.NewRequest(http.MethodPost, "/v1/ai-guard/compliance-webhook", nil)
	reqWithToken.Header.Set(rstokenauth.Header, "svc-token")
	rrWithToken := httptest.NewRecorder()
	mux.ServeHTTP(rrWithToken, reqWithToken)
	if rrWithToken.Code == http.StatusUnauthorized {
		t.Errorf("compliance-webhook with valid token: got 401, want auth to pass")
	}
}
...
func TestInitQuota_loadPoliciesWithMockPool(t *testing.T) {
	...
	engine, policyCache := InitQuota(context.Background(), db, nil, discardLogger(), nil)
	if engine == nil {
...
func testEncryptForCred(t *testing.T, keyHex, plaintext string, aad []byte) (ct, iv, tag string) {
	t.Helper()
...
func TestCredentialStoreAdapter_ResolveForProvider_withCredIDSuccess(t *testing.T) {
	...
	ctHex, ivHex, tagHex := testEncryptForCred(t, testKeyHex, plaintext, keyderive.ProviderCredentialAAD("cred-with-key", "prov-real"))
	...
	mgr := credmanager.NewManager(src, d)
	adapter := &credentialStoreAdapter{mgr: mgr}
