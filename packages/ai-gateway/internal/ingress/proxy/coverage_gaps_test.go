// coverage_gaps_test.go — additive tests targeting the remaining
// sub-95% surfaces inside the handler package without standing up the
// full broker + executor + canonical-bridge end-to-end pipeline.
//
// The big-three 0% blocks (handleNonStream / fetchUpstreamWithPreparedBody
// / runViaBroker and its three subscription handlers) require a real
// *executor.TargetExecutor + *streamcache.Registry + *canonicalbridge.Bridge;
// those are concrete pointer types (no interface seam), so test coverage
// of those entrypoints requires either (a) an in-package HTTP harness that
// runs the real upstream over httptest, or (b) refactoring proxy.go to
// introduce executor/broker/bridge interfaces. The constraint for this
// session is "STOP at test-only ceiling if seam scope balloons beyond
// established pattern", so we focus on every helper / branch reachable
// with the existing stubs.
package proxy
