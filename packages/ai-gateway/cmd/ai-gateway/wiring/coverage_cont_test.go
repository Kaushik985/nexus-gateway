// coverage_cont_test.go — coverage continuation tests for the wiring package.
// Targets functions left below 100% in the 51.7% → ≥95% push.
package wiring
...
func TestInitQuota_withNilPoolDB(t *testing.T) {
	...
	fakeDB := &store.DB{}
	engine, policyCache := InitQuota(context.Background(), fakeDB, nil, discardLogger())
	if engine == nil {
...
func TestMountAIGuardRoutes_registersRoutes(t *testing.T) {
	...
	MountAIGuardRoutes(
		mux, cfg, nil, nil, adapterReg, nil, nil, nil, configCache, discardLogger(),
	)
