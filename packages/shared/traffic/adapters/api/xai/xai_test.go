...
func TestExtractRequest_OpenAICompatDelegation(t *testing.T) {
	body := []byte(`{
		"model":"grok-2-latest",
		"messages":[{"role":"user","content":"hi from grok"}]
	}`)
	a := &Adapter{}
	...
	if len(nc.Segments) != 1 || nc.Segments[0] != "hi from grok" {
...
func TestDetectRequestMeta_XaiKeyClass(t *testing.T) {
	body := []byte(`{"messages":[{"role":"user","content":"hi"}]}`)
	a := &Adapter{}
	r, _ := http.NewRequest(http.MethodPost, "https://api.x.ai/v1/chat/completions", nil)
	r.Header.Set("Authorization", "Bearer xai-abcdef123456")
	meta := a.DetectRequestMeta(r, body)
	if meta.Provider != "xai" {
		t.Errorf("Provider=%q", meta.Provider)
	}
	if meta.ApiKeyClass != "xai-bearer" {
		t.Errorf("ApiKeyClass=%q", meta.ApiKeyClass)
	}
}

// Configure is a no-op for xai (delegates to openai inner which is also
...
// surface end-to-end through the xai wrapper.
func TestExtractResponse_ChatCompletionsDelegation(t *testing.T) {
	...
	if len(nc.Segments) != 1 || nc.Segments[0] != "hello from grok" {
		t.Errorf("Segments=%v", nc.Segments)
	}
...
// delta must produce a single Segment. The xai wrapper does no
// transform here.
...
// rewriter and substitutes per-message text — pin that the xai wrapper
// does not lose the rewritten output or the patch count.
...
// xai wrapper preserves the rewritten output and patch count.
...
// confirms the wrapper does not invent a stale "xai-bearer" tag.
func TestDetectRequestMeta_NoAuth(t *testing.T) {
	body := []byte(`{"model":"grok-2-latest","messages":[{"role":"user","content":"hi"}]}`)
	a := &Adapter{}
	r, _ := http.NewRequest(http.MethodPost, "https://api.x.ai/v1/chat/completions", nil)
	meta := a.DetectRequestMeta(r, body)
	if meta.ApiKeyClass != "" {
	...
	if meta.Model != "grok-2-latest" {
		t.Errorf("Model=%q", meta.Model)
	}
}

// DetectRequestMeta with a non-Bearer Authorization scheme: the wrapper
// must NOT stamp "xai-bearer" — non-Bearer headers are not API keys
// and tagging them would poison attribution.
...
// xai wrapper passes through unchanged, so prompt/completion split and
// Status=OK must surface end-to-end.
...
// Normalize: xai speaks the openai-chat wire shape. A canonical
// chat-completions body must claim Tier-1 with DetectedSpec=xai and
// surface the user prompt + model on the normalized payload.
func TestNormalize_OpenAIChatShape(t *testing.T) {
	...
	if payload.Model != "grok-2-latest" {
