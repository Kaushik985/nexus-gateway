...
// after the analytics file was added. Each test names the specific failure mode
// it exercises, per the unit-test-coverage-95 policy.
...
// UpdateDeviceGroup — bind error path (groups.go:121-123)
...
// PutGroupConfig — the json.Valid(req.State) branch at group_config.go:87
// is a defensive check against non-standard binders. When the standard
// Echo JSON binder parses successfully, json.RawMessage is always valid.
// The branch is already exercised by TestPutGroupConfig_InvalidStateJSON
// in group_config_test.go via backtick-embedded invalid state bytes.
// No additional test needed here.

// SetGroupMembershipQuery — updated == nil (not-found nil-check)
// groups_smart.go:126-128
...
// SetGroupMembershipQuery — json.Marshal error on predicate
// groups_smart.go:112-114 — structurally unreachable in practice
// (device.Predicate is json-serialisable) but the coverage gap is
// the errNotFound sentinel check vs a mock-injected error.
// Instead drive the groups_smart.go:120-122 errNotFound wrap branch.
...
// loadPreviewDevices + previewDevicesForSmartEval — pool path
// groups_smart.go:162 (fall-through) and 165+ (SQL scan)
...
// FleetAnalyticsTopDest — result non-nil (group mapping loop)
// fleet_analytics.go:61-71
...
// parseRFC3339Flexible — only RFC3339 non-Nano path (handler.go:169-171)
// The Nano branch (line 166) is exercised by TestParseRFC3339Flexible_Nano.
// The fallback-to-RFC3339 path fires when the string IS RFC3339 but NOT Nano.
// We confirm parseRFC3339Flexible("2026-05-17T10:00:00+05:30") takes the
// RFC3339 (non-Nano) branch.
