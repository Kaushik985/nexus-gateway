...
// logRelayResult — all 3 arms

func TestLogRelayResult_NilNoLog(t *testing.T) {
	t.Parallel()
	logRelayResult(discardLogger(), "x", nil) // nil → early return, no panic
}

func TestLogRelayResult_DialErrorWarn(t *testing.T) {
	t.Parallel()
	dialErr := &tlsbump.PassThroughError{Op: "dial", Host: "example.com:443", Err: errors.New("no route")}
	logRelayResult(discardLogger(), "kill", dialErr)
}

func TestLogRelayResult_CopyErrorDebug(t *testing.T) {
	t.Parallel()
	// Any non-dial PassThroughError → debug branch.
	copyErr := &tlsbump.PassThroughError{Op: "copy", Host: "example.com:443", Err: io.EOF}
	logRelayResult(discardLogger(), "kill", copyErr)
	// A plain (non-typed) error also exercises the debug branch.
	logRelayResult(discardLogger(), "kill", errors.New("peer closed"))
}
...
// TestStreamingPolicyStore_AtomicSwap locks the #115 Store-based
