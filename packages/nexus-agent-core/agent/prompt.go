...
func BuildSystemPrompt(in PromptInput) string {
	...
	b.WriteString("2. **Everything else** — any specific admin resource, config, or entity (virtual keys, providers, routing rules, hooks, IAM, caches, jobs, …) — go through the resource catalog SEARCH-FIRST: resource_search (a ranked keyword search over every admin operation — pass the words from the question). The top candidates come back as full cards — summary, params, body skeleton — so a matching card is DIRECTLY executable: call resource_read (GET) or resource_invoke (write) with it in the same turn. Reach for resource_describe only when no card covers the target (a thin `more` entry looks right, or you need a kind's whole surface). The catalog has ~380 operations; you reach them by searching, never by guessing a path, and they are never all listed for you.\n\n")
	...
	b.WriteString("- Filter first: narrow on the server with the query parameters the search card (or resource_describe) lists — each param carries its meaning — rather than listing everything and scanning it by eye.\n\n")
