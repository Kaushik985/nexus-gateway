...
**Create and detail.** The form selects a protocol — OIDC or SAML. An OIDC provider collects a name, an enabled flag, the issuer, client id, client secret (masked on edit), redirect URI, scopes (defaulting to `openid profile email`), and an optional list of **extra authorize parameters** (key/value pairs appended to the sign-in request, e.g. Auth0 `organization` or `prompt`); the authorize, token, and JWKS endpoints are discovered from the issuer automatically. A SAML provider collects a name, an enabled flag, an entity id, an SSO URL, and a certificate. Both protocols also set two just-in-time provisioning defaults: a **default role** (the IAM group every auto-provisioned user joins as a baseline, picked from the existing groups) and a **Grant Control Plane access by default** switch. A test action probes the candidate or saved configuration. On the detail page, two further features attach to a provider: mapping an external IdP group to a role, and managing the provider's SCIM tokens.

**Key concepts.** The protocol is `oidc` or `saml`. The group claim and the external-group-to-role mapping let SSO group membership drive Nexus role membership; the default role gives every federated user a baseline group even when no external group maps. The "Grant Control Plane access by default" switch is per-provider because one IdP may federate both Control Plane admins and agent end-users — leave it off for the latter.
