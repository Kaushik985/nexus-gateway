// Package chatglmweb implements the chatglm-web traffic adapter for
// browser-side traffic to chatglm.cn (Zhipu's consumer ChatGLM web
// product). Distinct from open.bigmodel.cn (handled by the glm
// adapter — programmatic API).
//
// Defensive JSON-aware adapter.
package chatglmweb
...
func (a *Adapter) DetectRequestMeta(_ *http.Request, body []byte) traffic.RequestMeta {
	meta := traffic.RequestMeta{Provider: "chatglm-web"}
	if !gjson.ValidBytes(body) {
		return meta
	}
