...
`rollup-correction` (`rollup_correction.go`) handles late-arriving events. Events can land in `traffic_event` after their bucket has already been rolled up; once per day the correction job recomputes every five-minute bucket of the previous day, then re-merges that day's `1h` and `1d` layers (and the `1mo` layer when the day was a month boundary). It delegates to the same `rollup-5m` and merge jobs so the aggregation logic lives in exactly one place, and it deliberately does not touch the live watermark path.
