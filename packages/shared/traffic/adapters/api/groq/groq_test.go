...
func TestExtractRequest_OpenAICompatDelegation(t *testing.T) {
	...
	a := &Adapter{}
	nc, err := a.ExtractRequest(context.Background(), body, "/openai/v1/chat/completions")
	if err != nil {
		t.Fatalf("err=%v", err)
	}
	if len(nc.Segments) != 1 || nc.Segments[0] != "hi from groq" {
...
func TestExtractRequest_ToolCallsDelegation(t *testing.T) {
	...
	if len(nc.ToolCallSegments) != 1 || !strings.Contains(nc.ToolCallSegments[0], `"calculator"`) {
		t.Errorf("ToolCallSegments=%v", nc.ToolCallSegments)
	}
}

func TestDetectRequestMeta_GroqKeyClass(t *testing.T) {
	body := []byte(`{"messages":[{"role":"user","content":"hi"}]}`)
	a := &Adapter{}
	r, _ := http.NewRequest(http.MethodPost, "https://api.groq.com/openai/v1/chat/completions", nil)
	r.Header.Set("Authorization", "Bearer gsk_abcdef123456")
	meta := a.DetectRequestMeta(r, body)
	if meta.Provider != "groq" {
		t.Errorf("Provider=%q", meta.Provider)
	}
	if meta.ApiKeyClass != "groq-bearer" {
		t.Errorf("ApiKeyClass=%q", meta.ApiKeyClass)
	}
}

// Configure is a no-op for groq (delegates to openai inner which is also
...
// surface end-to-end through the groq wrapper.
func TestExtractResponse_ChatCompletionsDelegation(t *testing.T) {
	...
	a := &Adapter{}
	nc, err := a.ExtractResponse(context.Background(), body, "/openai/v1/chat/completions")
	if err != nil {
		t.Fatalf("err=%v", err)
	}
	if len(nc.Segments) != 1 || nc.Segments[0] != "hello from groq" {
		t.Errorf("Segments=%v", nc.Segments)
	}
...
func TestExtractResponse_Malformed(t *testing.T) {
	a := &Adapter{}
	_, err := a.ExtractResponse(context.Background(), []byte(`not json`), "/openai/v1/chat/completions")
...
// delta must produce a single Segment. The groq wrapper does no
// transform here.
func TestExtractStreamChunk_ContentDelta(t *testing.T) {
	chunk := []byte(`{"choices":[{"index":0,"delta":{"content":"hi"}}]}`)
	a := &Adapter{}
	nc, err := a.ExtractStreamChunk(context.Background(), chunk, "/openai/v1/chat/completions")
	if err != nil {
		t.Fatalf("err=%v", err)
	}
...
func TestExtractStreamChunk_Malformed(t *testing.T) {
	a := &Adapter{}
	_, err := a.ExtractStreamChunk(context.Background(), []byte(`not json`), "/openai/v1/chat/completions")
	if !errors.Is(err, traffic.ErrMalformed) {
		t.Errorf("err=%v want ErrMalformed", err)
	}
}
...
// rewriter and substitutes per-message text — pin that the groq
...
func TestRewriteRequestBody_ChatCompletionsDelegation(t *testing.T) {
	...
	a := &Adapter{}
	out, n, err := a.RewriteRequestBody(context.Background(), body, "/openai/v1/chat/completions", traffic.NormalizedContent{
...
func TestRewriteRequestBody_EmbeddingsUnsupported(t *testing.T) {
	a := &Adapter{}
	_, _, err := a.RewriteRequestBody(context.Background(), []byte(`{"input":"hi"}`), "/openai/v1/embeddings", traffic.NormalizedContent{})
...
// groq wrapper preserves the rewritten output and patch count.
func TestRewriteResponseBody_ChatCompletionsDelegation(t *testing.T) {
	...
	a := &Adapter{}
	out, n, err := a.RewriteResponseBody(context.Background(), body, "/openai/v1/chat/completions", traffic.NormalizedContent{
		Segments: []string{"REDACTED"},
	})
	if err != nil {
		t.Fatalf("err=%v", err)
...
func TestRewriteResponseBody_EmbeddingsUnsupported(t *testing.T) {
	a := &Adapter{}
	_, _, err := a.RewriteResponseBody(context.Background(), []byte(`{"data":[]}`), "/openai/v1/embeddings", traffic.NormalizedContent{})
	if !errors.Is(err, traffic.ErrRewriteUnsupported) {
		t.Errorf("err=%v want ErrRewriteUnsupported", err)
	}
}
...
// confirms the wrapper does not invent a stale "groq-bearer" tag.
func TestDetectRequestMeta_NoAuth(t *testing.T) {
	body := []byte(`{"model":"llama-3.3-70b-versatile","messages":[{"role":"user","content":"hi"}]}`)
	a := &Adapter{}
	r, _ := http.NewRequest(http.MethodPost, "https://api.groq.com/openai/v1/chat/completions", nil)
	meta := a.DetectRequestMeta(r, body)
	if meta.ApiKeyClass != "" {
...
// must NOT stamp "groq-bearer" — non-Bearer headers are not API
...
func TestDetectRequestMeta_NilRequest(t *testing.T) {
	body := []byte(`{"model":"llama-3.3-70b-versatile"}`)
	a := &Adapter{}
...
// groq wrapper passes through unchanged, so prompt/completion split
...
// Normalize: groq speaks the openai-chat wire shape. A canonical
// chat-completions body must claim Tier-1 with DetectedSpec=groq
