...
import { complianceApi } from '../../../../src/api/services/compliance/compliance';
...
describe('complianceApi', () => {
  ...
  it('listAuditEvents builds the full query string from every filter', async () => {
    await complianceApi.listAuditEvents({
      source: 'agent',
      hookDecision: 'block',
      complianceTags: ['pii', 'secret'],
      sourceIp: '1.2.3.4',
    ...
    expect(getSpy).toHaveBeenCalledWith(
      '/api/admin/compliance/audit?source=agent&hookDecision=block&complianceTags=pii%2Csecret&sourceIp=1.2.3.4&targetHost=api.openai.com&startTime=S&endTime=E&limit=50&offset=10',
    );
  });
  it('listAuditEvents omits the query string when no filters are passed', async () => {
    await complianceApi.listAuditEvents();
    expect(getSpy).toHaveBeenCalledWith('/api/admin/compliance/audit');
  });
  it('getAuditEvent GETs /api/admin/compliance/audit/:id', async () => {
    await complianceApi.getAuditEvent('a1');
    expect(getSpy).toHaveBeenCalledWith('/api/admin/compliance/audit/a1');
  });
  it('getTrinityStats threads the time window through the query', async () => {
    await complianceApi.getTrinityStats('S', 'E');
    expect(getSpy).toHaveBeenCalledWith('/api/admin/compliance/trinity?startTime=S&endTime=E');
  });
  it('getTrinityStats omits the query when no window is given', async () => {
    await complianceApi.getTrinityStats();
    expect(getSpy).toHaveBeenCalledWith('/api/admin/compliance/trinity');
  });
  it('getOverview always sends both startTime + endTime', async () => {
    await complianceApi.getOverview('S', 'E');
    expect(getSpy).toHaveBeenCalledWith('/api/admin/compliance/overview?startTime=S&endTime=E');
  });
  it('buildOverviewExportUrl returns a query-string URL without calling the client', () => {
    const url = complianceApi.buildOverviewExportUrl({ startTime: 'S', endTime: 'E' });
    expect(url).toBe('/api/admin/compliance/overview/export?startTime=S&endTime=E');
