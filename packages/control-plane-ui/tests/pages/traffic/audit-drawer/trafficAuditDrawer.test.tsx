...
import { TrafficEventDrawer } from '../../../../src/pages/traffic/audit-drawer/trafficAuditDrawer';
...
// ── Tab navigation + drawer chrome ───────────────────────────────────────────

describe('TrafficEventDrawer — tab navigation', () => {
  const FULL_EVENT = makeEvent({
    statusCode: 200,
    promptTokens: 100, completionTokens: 50, totalTokens: 150,
    requestBody: '{"model":"gpt-4o"}',
    requestHooksPipeline: [{ name: 'pii', decision: 'APPROVE' }],
  });

  beforeEach(() => { mockGetTrafficEvent.mockResolvedValue(FULL_EVENT); });

  it('defaults to the Overview tab showing the basic-info section', async () => {
    renderDrawer(FULL_EVENT);
    await waitFor(() => expect(screen.getByText(i18n.t('pages:traffic.detail.basic.title'))).toBeInTheDocument());
  });

  it('AI & Routing tab reveals the token-usage section', async () => {
    renderDrawer(FULL_EVENT);
    fireEvent.click(await screen.findByTestId('audit-drawer-tab-ai'));
    await waitFor(() => expect(screen.getByText(i18n.t('pages:traffic.detail.aiProvider.tokenUsageTitle'))).toBeInTheDocument());
  });

  it('Compliance tab reveals the hook-pipeline section', async () => {
    renderDrawer(FULL_EVENT);
    fireEvent.click(await screen.findByTestId('audit-drawer-tab-compliance'));
    await waitFor(() => expect(screen.getByText(i18n.t('pages:traffic.detail.compliance.title'))).toBeInTheDocument());
  });

  it('Payloads tab toggles between the normalized and raw sub-views', async () => {
    renderDrawer(FULL_EVENT);
    fireEvent.click(await screen.findByTestId('audit-drawer-tab-payloads'));
    const normalizedBtn = await screen.findByTestId('payload-subtab-normalized');
    const rawBtn = screen.getByTestId('payload-subtab-raw');
    expect(normalizedBtn).toBeInTheDocument();
    fireEvent.click(rawBtn); // switch to raw view
    fireEvent.click(normalizedBtn); // back to normalized
    expect(screen.getByText(i18n.t('pages:traffic.detail.payload.requestBody'))).toBeInTheDocument();
  });

  it('close button + overlay both invoke onClose', async () => {
    const onClose = vi.fn();
    renderWithRouter(
      <TrafficEventDrawer
        selectedEntry={FULL_EVENT as Parameters<typeof TrafficEventDrawer>[0]['selectedEntry']}
        drawerVisible
        onClose={onClose}
      />,
    );
    fireEvent.click(await screen.findByLabelText(i18n.t('pages:traffic.detail.closeAriaLabel')));
    expect(onClose).toHaveBeenCalledTimes(1);
    // the dimmed overlay (role=presentation, aria-hidden) also closes the drawer
