...
// Coverage tests for bedrock. Whitebox (internal) package so we can
// exercise the unexported `errorNormalizer`, the codec error paths, and
// the Transport helper methods (BuildURL / ApplyAuth / Do / Probe) that
// the external spec_test.go already covers via the SpecAdapter wrapper.
//
...
func TestCodec_DecodeResponse_DelegatesToAnthropic(t *testing.T) {
	...
	decRes, err := c.DecodeResponse(typology.WireShapeBedrockConverse, native, "")
...
// TestApplyAuth_SessionTokenForwarded asserts optional session token
// rides through the X-Nexus-Bedrock-* envelope (consumed by Do).
func TestApplyAuth_SessionTokenForwarded(t *testing.T) {
	tr := NewTransport(slog.Default())
	r := httptest.NewRequest(http.MethodPost, "https://x/model/m/invoke", nil)
	err := tr.ApplyAuth(r, provcore.CallTarget{
	...
	if got := r.Header.Get("X-Nexus-Bedrock-SessionToken"); got != "tok" {
		t.Errorf("sessionToken header=%q", got)
	}
}

// TestApplyAuth_NoSessionToken asserts the header is absent when the
// caller did not supply one (covers the `if st != ""` skip branch).
func TestApplyAuth_NoSessionToken(t *testing.T) {
	tr := NewTransport(slog.Default())
	r := httptest.NewRequest(http.MethodPost, "https://x/model/m/invoke", nil)
	err := tr.ApplyAuth(r, provcore.CallTarget{
		Extras: map[string]string{"aws.region": "us-east-1", "aws.accessKey": "a", "aws.secretKey": "s"},
	...
	if r.Header.Get("X-Nexus-Bedrock-SessionToken") != "" {
		t.Errorf("sessionToken header should be unset when absent")
	}
}

// transport.go — Do.

// TestDo_MissingInternalSignerState covers the early-fail branch when
// the X-Nexus-Bedrock-* headers were stripped (or never set) — Do
// should not silently dispatch unsigned.
func TestDo_MissingInternalSignerState(t *testing.T) {
	tr := NewTransport(slog.Default())
	r := httptest.NewRequest(http.MethodPost, "https://example.invalid/invoke", strings.NewReader(`{}`))
	// no X-Nexus-Bedrock-* headers set → must fail
	resp, err := tr.Do(context.Background(), r)
	if resp != nil {
		_ = resp.Body.Close()
	}
	if err == nil || !strings.Contains(err.Error(), "internal signer state missing") {
		t.Fatalf("expected internal-state error, got %v", err)
	}
}
...
func TestDo_BodyReadError(t *testing.T) {
	...
	// Re-stamp the internal headers ApplyAuth would have set.
	r.Header.Set("X-Nexus-Bedrock-AccessKey", "a")
	r.Header.Set("X-Nexus-Bedrock-SecretKey", "s")
	r.Header.Set("X-Nexus-Bedrock-Region", "us-east-1")
	resp, err := tr.Do(context.Background(), r)
	if resp != nil {
		_ = resp.Body.Close()
	}
...
// TestDo_StripsInternalHeadersFromUpstream asserts the X-Nexus-Bedrock-*
// envelope is removed before the request hits the wire — those headers
// would leak credentials to the upstream + would break SigV4 (signed
// header set ↔ wire header set must match).
func TestDo_StripsInternalHeadersFromUpstream(t *testing.T) {
	var saw http.Header
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		saw = r.Header.Clone()
		w.WriteHeader(http.StatusOK)
	...
	r.Header.Set("X-Nexus-Bedrock-SecretKey", "s")
	r.Header.Set("X-Nexus-Bedrock-Region", "us-east-1")
	r.Header.Set("X-Nexus-Bedrock-SessionToken", "tok")
	resp, err := tr.Do(context.Background(), r)
	if err != nil {
		t.Fatalf("Do: %v", err)
	}
	...
	for _, h := range []string{
		...
		if saw.Get(h) != "" {
			t.Errorf("internal header %q leaked to upstream: %q", h, saw.Get(h))
		}
	}
	...
	// Session-token path also sets X-Amz-Security-Token by the AWS signer.
