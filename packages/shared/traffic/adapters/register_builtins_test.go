...
// (`return &xxx.Adapter{}`) for adapters without a Normalize method
// (which the Tier-1 normalizer loop never instantiates into the
// registry) are never executed and stay uncovered. This pins two
// contracts at once:
...
// TestTier1AdapterNormalizers_DisjointFromCodecKeys mechanizes the
// lock-step guard documented on RegisterTier1AdapterNormalizers: the
// per-host adapter Normalizer IDs must stay disjoint from the wire-
// format keys the shared codecs claim in RegisterDefaultAIBuiltins.
// Assembling both halves into one registry — exactly what every
// service does at startup — panics on the first collision, so this
// test fails the moment someone adds a Normalize method to a
// standard-API vendor adapter whose ID a codec already owns.
func TestTier1AdapterNormalizers_DisjointFromCodecKeys(t *testing.T) {
	t.Parallel()
	reg := normalizecore.NewRegistry()
	codecs.RegisterDefaultAIBuiltins(reg)
	// Panics with "duplicate registration" on any ID overlap.
