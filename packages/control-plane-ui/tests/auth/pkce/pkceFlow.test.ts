...
describe('consumeCallback', () => {
  ...
  it('treats a corrupt stored PKCE blob as no pending flow', async () => {
    sessionStorage.setItem('nexus_pkce_state', '{ not valid json');
    // readStoredState's JSON.parse throws → caught → null → same "no flow" error
    await expect(consumeCallback('?code=c&state=s')).rejects.toThrow(/No pending OAuth flow/);
  });

  it('dedupes concurrent calls for the same callback URL (returns the same promise)', async () => {
    __setStoredStateForTests({ verifier: 'verifier-xyz', state: 'state-abc', postAuthPath: '/' });
    server.use(
      http.post('/oauth/token', () =>
        HttpResponse.json({ access_token: 'at-d', refresh_token: 'rt-d', token_type: 'Bearer', expires_in: 3600 }),
      ),
    );
    const search = '?code=c&state=state-abc';
    const p1 = consumeCallback(search);
    const p2 = consumeCallback(search);
    expect(p1).toBe(p2); // second call returns the cached in-flight promise
    const r = await p1;
    expect(r.tokens.accessToken).toBe('at-d');
  });
