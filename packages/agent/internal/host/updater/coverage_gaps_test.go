...
// signEd25519 produces (sha256-hex, ed25519-sig-base64) for the given content.
func signEd25519(t *testing.T, priv ed25519.PrivateKey, content []byte) (string, string) {
	t.Helper()
...
func TestCheckAndUpdate_EmptySignature_Rejected(t *testing.T) {
	...
	if err == nil || !errContains(err, "signature") {
		t.Fatalf("must reject when server omits Ed25519 signature: %v", err)
	}
	if _, statErr := os.Stat(bin + ".tmp"); statErr == nil {
		t.Fatal(".tmp file should be removed when signature is missing")
	}
}
...
func TestCheckAndUpdate_BadSignatureBase64_Rejected(t *testing.T) {
	...
	_, err := u.CheckAndUpdate(context.Background())
	if err == nil || !errContains(err, "signature verification failed") {
		t.Fatalf("invalid base64 must surface via signature-verification wrapper: %v", err)
	}
	if _, statErr := os.Stat(bin + ".tmp"); statErr == nil {
		t.Fatal(".tmp file should be removed on signature decode error")
	}
}

func TestCheckAndUpdate_SignatureMismatch_Rejected(t *testing.T) {
	pub, _, _ := ed25519.GenerateKey(nil)
	_, otherPriv, _ := ed25519.GenerateKey(nil) // signed with a DIFFERENT key
	body := []byte("payload")
	hashHex, sigB64 := signEd25519(t, otherPriv, body)
	...
	_, err := u.CheckAndUpdate(context.Background())
	if err == nil || !errContains(err, "signature verification failed") {
		t.Fatalf("wrong-key signature must be rejected: %v", err)
	}
	got, _ := os.ReadFile(bin)
	if string(got) != "old" {
		t.Fatalf("original binary must be preserved on signature mismatch, got %q", string(got))
	}
}
...
func TestRunWithAvailabilityCallback_EnabledAutoInstallReturnsAfterApply(t *testing.T) {
	...
	hashHex, sigB64 := signEd25519(t, priv, body)
	...
	client := &recordingChecker{
		responses: []hub.UpdateInfo{
			{Available: false},
			{Available: true, Version: "2.0", DownloadURL: srv.URL, SHA256: hashHex, Signature: sigB64},
		},
	}
	u := NewUpdater(client, Config{Enabled: true, CheckInterval: 20 * time.Millisecond, PublicKey: pub}, "1.0.0", "darwin", bin)
...
func TestCheckAndUpdate_PublicKeyNil_Rejected(t *testing.T) {
	...
	// tmp file cleaned up.
	if _, statErr := os.Stat(bin + ".tmp"); statErr == nil {
		t.Fatal(".tmp must be removed when public key is missing")
	}
}
...
func TestCheckAndUpdate_AtomicSwapFails(t *testing.T) {
	...
	// "backup current binary" which CheckAndUpdate wraps as "atomic swap".
	pub, priv, _ := ed25519.GenerateKey(nil)
	body := []byte("v2-body")
	hashHex, sigB64 := signEd25519(t, priv, body)
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, _ *http.Request) {
	...
	u := NewUpdater(client, Config{Enabled: true, PublicKey: pub}, "1.0.0", "darwin", missingBin)

	_, err := u.CheckAndUpdate(context.Background())
	if err == nil || !errContains(err, "atomic swap") {
		t.Fatalf("expected atomic-swap wrapper; got %v", err)
	}
