# Runbook — "Chat with Nexus" web assistant
...
accepted design decision). Per-user token / cost ceilings are **not** implemented;
assistant inference is bounded by the system VK's own gateway-side rate-limit/quota,
per-session turn serialization (HTTP 409 `turn_in_progress`), a **per-user
concurrent-turn cap** (HTTP 429 `user_turn_limit_exceeded` — one user cannot starve
the shared key), the wall-clock turn deadline, and the agent step cap. Per-user admin **actions** are still
...
Endpoints (all under `/api/admin/assistant/`, behind `AdminAuth` **plus** the
dedicated assistant IAM actions — `admin:assistant.read` for the GET/stream surface,
`admin:assistant.write` for turns/confirms/interrupt/delete; login alone cannot spend
the system VK. Every tool is additionally IAM-checked at the admin API it
self-calls). A turn is
...
| `NEXUS_ASSISTANT_PROD` | **yes on production** | `1` arms the production posture: the agent's prompt states it is on prod AND every dangerous write requires the backend-enforced second confirmation. Unset, the assistant presents (and behaves) as non-prod — the boot log warns loudly. |
...
## Session affinity (REQUIRED for multi-replica)
The stream itself reconnects across blips (`GET …/stream?lastSeq=` replays missed
events), but the confirm registry that pairs a parked write with its `POST /confirm`
decision is **in-process, per-pod**. A multi-replica deployment therefore **must pin a
user's assistant requests to the same pod**:
...
## SSE transport
`GET /sessions/:id/stream` is the SSE channel. The handler sets
`Content-Type: text/event-stream`, `Cache-Control: no-cache`,
`Connection: keep-alive`, and `X-Accel-Buffering: no`. Any reverse proxy in
front of the CP must **not buffer** the response:
...
- Load balancers: disable response buffering / raise idle timeouts on the stream path.
A dropped stream starts the disconnect-grace timer; a reconnect within it resumes
(`?lastSeq=` replays missed events), otherwise the turn is cancelled server-side
(stopping the agent + system-VK billing). A deliberate Stop interrupts immediately.
...
1. `NEXUS_ASSISTANT_SYSTEM_VK` is set and the VK is valid on the AI Gateway
   (a trivial chat turn streams a reply, not a 503).
...
4. Reverse proxy does not buffer the stream path (tokens stream incrementally, not
...
6. On production: `NEXUS_ASSISTANT_PROD=1` is set — the boot log prints
   "assistant production posture: ENABLED" (a warn-level "disabled" line on a prod
   deploy is a misconfiguration; the second confirm would not be armed).
...
|---|---|---|
| `assistant_turns_total` | `result` = ok \| error \| aborted \| unavailable \| unsupported_auth \| user_limit | per-turn outcome (user_limit = the per-user concurrent-turn 429) |
...
| `assistant_navigations_total` | — | cross-page navigation directives emitted (0..n per turn — a single turn can emit several or none) |
| `assistant_pii_to_prompt_total` | — | tool results in which PII was redacted before prompt entry (guardrail target 0) |
| `assistant_confirm_misroute_total` | — | confirm POSTs answered 421 (another instance owns the session) — a sustained rate means ingress affinity is misconfigured |
...
Not yet emitted (honest gaps): the **North-Star success refinement**
...
## Related
- Architecture: `docs/developers/architecture/nexus-operator-toolkit-architecture.md`
  (the "web face" section covers the per-turn agent build, the command/data-stream
  split, the in-process self-call + audit `via` stamp, and the chat audit chain).
- Feature doc: `docs/users/features/cp-ui/web-assistant.md`.
- API: `docs/users/api/openapi/control-plane/assistant.yaml`.
- Persistence as-built: the basic-chat persistence tables in
