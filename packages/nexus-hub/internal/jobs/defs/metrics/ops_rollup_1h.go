...
const (
	opsRollup1hJobID          = "ops-rollup-1h"
	opsRollup1hJobName        = "Ops Metrics Rollup (1 hour)"
	opsRollup1hJobDescription = "Aggregates metric_ops_raw into metric_ops_rollup_1h every 5 minutes. Services keep per-instance rows; agents keep per-instance rows only when in diagnostic mode for the bucket and otherwise collapse into a single fleet-aggregate row (thing_id IS NULL) per metric+dimension."

	opsWatermarkName = "ops_1h"
	opsBucketDur     = time.Hour
	// opsSealedGrace defers processing of an hour until at least 5 minutes
	// after it ends so straggler samples (clock skew, batch flush latency)
	// land in the rollup. Mirrors the 1h merge job's natural seal margin.
	opsSealedGrace = 5 * time.Minute

	opsRollupTable = "metric_ops_rollup_1h"
	opsRawTable    = "metric_ops_raw"
)

// OpsRollup1hJob aggregates metric_ops_raw rows into metric_ops_rollup_1h.
...
// NewOpsRollup1h constructs the job. interval defaults to 5 minutes (the
// scheduler's natural rollup tick — short enough that fresh data appears in
// the UI within a few minutes of the bucket sealing).
func NewOpsRollup1h(pool *pgxpool.Pool, interval time.Duration, logger *slog.Logger) *OpsRollup1hJob {
	if interval <= 0 {
		interval = 5 * time.Minute
	}
	return &OpsRollup1hJob{
		pool:     pool,
		interval: interval,
		logger:   logger.With("job", opsRollup1hJobID),
	}
}

func (j *OpsRollup1hJob) ID() string              { return opsRollup1hJobID }
func (j *OpsRollup1hJob) Name() string            { return opsRollup1hJobName }
func (j *OpsRollup1hJob) Description() string     { return opsRollup1hJobDescription }
func (j *OpsRollup1hJob) Interval() time.Duration { return j.interval }
...
func (j *OpsRollup1hJob) RunOnStart() bool { return true }

func (j *OpsRollup1hJob) Run(ctx context.Context) error {
...
func (j *OpsRollup1hJob) resolveCursor(ctx context.Context) (time.Time, bool, error) {
	...
	// If watermark is in the future (the d86ca4f6 NOW() seed) and oldest
	// pre-dates it, prefer the bootstrap cursor so we don't skip data.
	// Otherwise advance to the hour after the watermark.
...
func (j *OpsRollup1hJob) minSampledAt(ctx context.Context) (time.Time, bool, error) {
...
// metric_ops_rollup_1h within a single transaction that also advances the
// watermark.
func (j *OpsRollup1hJob) processOneBucket(ctx context.Context, bucket time.Time) error {
...
func (j *OpsRollup1hJob) deleteBucket(ctx context.Context, tx pgx.Tx, bucket time.Time) error {
...
func (j *OpsRollup1hJob) diagAgentsInWindow(ctx context.Context, tx pgx.Tx, bucket, bucketEnd time.Time) ([]string, error) {
...
// insertScalarPerThing aggregates non-histogram rows into metric_ops_rollup_1h
...
func (j *OpsRollup1hJob) insertScalarPerThing(ctx context.Context, tx pgx.Tx, bucket, bucketEnd time.Time, diagAgents []string) error {
	const q = `
...
// NULL.
func (j *OpsRollup1hJob) insertScalarFleet(ctx context.Context, tx pgx.Tx, bucket, bucketEnd time.Time, diagAgents []string) error {
	const q = `
		INSERT INTO metric_ops_rollup_1h
			(id, bucket_start, thing_id, thing_type, metric_name, metric_kind, dimension_key,
			 value_avg, value_sum, value_min, value_max, sample_count, metadata)
...
// metric, dim).
func (j *OpsRollup1hJob) insertHistograms(ctx context.Context, tx pgx.Tx, bucket, bucketEnd time.Time, diagAgents []string) error {
	rows, err := tx.Query(ctx, `
