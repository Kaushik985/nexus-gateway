...
func keyVKs(t *testing.T, v *vks, key string) (*vks, tea.Cmd) {
	t.Helper()
	m, cmd := v.Update(tea.KeyMsg{Type: tea.KeyRunes, Runes: []rune(key)})
	return m.(*vks), cmd
}
...
func TestVKs_RevokeNonProd(t *testing.T) {
	...
	v, cmd := keyVKs(t, v, "r")
	if cmd == nil {
		t.Fatal("non-prod revoke should fire immediately")
	}
	m, _ := v.Update(cmd())
	v = m.(*vks)
...
func TestVKs_RevokeGatedOnActive(t *testing.T) {
	...
	v := newVKs(gw, Session{EnvName: "prod", IsProd: true})
	v.Update(v.Init()())
	v, cmd := keyVKs(t, v, "r")
	if cmd != nil {
...
func TestVKs_RevokeError(t *testing.T) {
	...
	v, cmd := keyVKs(t, v, "r")
	if cmd == nil {
		t.Fatal("revoke should fire a command")
	}
	m, _ := v.Update(cmd())
	v = m.(*vks)
...
func TestVKs_RegenerateShowsSecretOnce(t *testing.T) {
	...
	v, cmd := keyVKs(t, v, "g")
	if cmd == nil {
		t.Fatal("non-prod regenerate should fire immediately")
	}
	m, _ := v.Update(cmd())
	v = m.(*vks)
	...
	// esc dismisses the panel back to the list.
...
func TestRouting_ViewAndToggle(t *testing.T) {
	...
	// non-prod toggle fires immediately and disables the enabled rule.
	m, cmd := v.Update(tea.KeyMsg{Type: tea.KeyRunes, Runes: []rune("t")})
	v = m.(*routing)
	if cmd == nil {
		t.Fatal("non-prod toggle should fire immediately")
	}
	m, _ = v.Update(cmd())
	v = m.(*routing)
...
func TestRouting_ProdConfirmAndError(t *testing.T) {
	...
	v := newRouting(gw, Session{EnvName: "prod", IsProd: true})
	v.Update(v.Init()())
	m, _ := v.Update(tea.KeyMsg{Type: tea.KeyRunes, Runes: []rune("t")})
	v = m.(*routing)
	if !v.capturing() || gw.lastRuleEnabled != nil {
		t.Fatal("prod toggle must require confirmation before firing")
	}
	if !strings.Contains(v.help(), "prod") {
	...
	ev.loading = false
	m, cmd = ev.Update(tea.KeyMsg{Type: tea.KeyRunes, Runes: []rune("t")})
	ev = m.(*routing)
	m, _ = ev.Update(cmd())
	ev = m.(*routing)
...
func TestVKs_RenderBranches(t *testing.T) {
	...
	pv := newVKs(&fakeGateway{vks: []core.VirtualKey{{ID: "a", Name: "a", Enabled: true, VKStatus: strPtr("active")}}}, Session{EnvName: "prod", IsProd: true})
	pv.Update(pv.Init()())
	pv, _ = keyVKs(t, pv, "r")
	if !strings.Contains(pv.help(), "prod") {
		t.Fatalf("confirm help should mention the env name: %q", pv.help())
	}
...
func TestWave3_InFlightGuard(t *testing.T) {
	...
	v, cmd := keyVKs(t, v, "g")
	if cmd == nil || !v.busy {
		t.Fatal("first regenerate should dispatch and mark the view busy")
	}
	...
	r := newRouting(rgw)
	r.Update(r.Init()())
	m2, rcmd := r.Update(tea.KeyMsg{Type: tea.KeyRunes, Runes: []rune("t")})
	r = m2.(*routing)
	if rcmd == nil || !r.busy {
		t.Fatal("first toggle should dispatch and mark busy")
	}
	if _, rcmd2 := r.Update(tea.KeyMsg{Type: tea.KeyRunes, Runes: []rune("t")}); rcmd2 != nil {
...
func TestWave3_NoSelectionAndFirstError(t *testing.T) {
	...
	er.Update(er.Init()())
	if m, cmd := er.Update(tea.KeyMsg{Type: tea.KeyRunes, Runes: []rune("t")}); cmd != nil {
		_ = m
