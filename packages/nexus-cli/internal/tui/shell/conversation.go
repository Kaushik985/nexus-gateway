...
type conversation struct {
	...
	// resume is the session the next agent (re)build continues — set by a
	// /sessions pick, cleared by /clear. The agent mutates this object in place
	// turn over turn, so a later rebuild (e.g. a /model switch) keeps the full
	// transcript and the same session id.
...
func (c *conversation) ensureAgent() error {
	...
	stream := AgentStream{
		...
		OnToolStart: func(name string, input []byte) { b.sendEv(agentToolMsg{name: name, input: input}) },
	...
	runner, err := c.build(b, b.confirm, stream, c.resume)
	if err != nil {
		c.buildErr = err
		return err
	}
