...
func TestPostJSON_TransportError(t *testing.T) {
	...
	if err == nil {
		t.Fatal("postJSON must surface a (generic) error from c.Do failure")
	}
	if sc != 0 {
	...
	if errors.Is(err, sentinel) || strings.Contains(err.Error(), "simulated transport boom") {
		t.Errorf("raw transport error leaked through the oracle collapse: %v", err)
	}
	if err.Error() != "alert delivery failed" {
		t.Errorf("err=%q, want generic 'alert delivery failed'", err.Error())
	}
}
...
func TestWebhookSender_TransportError(t *testing.T) {
	...
	sc, err := w.Send(context.Background(), ch, alerting.Alert{ID: "x"})
	if err == nil {
		t.Fatal("expected an error from c.Do failure")
	}
	if sc != 0 {
