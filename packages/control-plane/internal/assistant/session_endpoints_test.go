...
// TestStartChat_RejectsAtUserConcurrencyCap is the handler half of the per-user
// gate: once a user has maxConcurrentTurnsPerUser turns in flight across DISTINCT
// sessions, a chat on a BRAND-NEW session is refused 429 user_turn_limit_exceeded —
// distinct from the per-session 409. This bounds one user's draw on the shared system VK.
func TestStartChat_RejectsAtUserConcurrencyCap(t *testing.T) {
	h := New(Config{SystemVK: "nvk_test", Model: "m"})
	// Saturate the user's slots with distinct in-flight sessions.
	for i := range maxConcurrentTurnsPerUser {
		h.bus.startTurn("u-1:busy"+strconv.Itoa(i), context.Background(), time.Minute)
	}
	rec, c := ctxFor(http.MethodPost, "/api/admin/assistant/sessions/fresh/chat", "u-1", "fresh", `{"message":"hi"}`)
	if err := h.StartChat(c); err != nil {
		t.Fatal(err)
	}
	if rec.Code != http.StatusTooManyRequests {
		t.Fatalf("a new session past the per-user cap must be 429, got %d (%s)", rec.Code, rec.Body.String())
	}
	if !strings.Contains(rec.Body.String(), "user_turn_limit_exceeded") {
		t.Fatalf("429 body must name user_turn_limit_exceeded, got %s", rec.Body.String())
	}
}
