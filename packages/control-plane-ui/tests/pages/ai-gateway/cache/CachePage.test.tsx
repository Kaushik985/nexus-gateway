...
import { CachePage } from '../../../../src/pages/ai-gateway/cache/CachePage';
...
describe('CachePage — StatusStrip', () => {
  ...
  it('emergency-disabling the semantic cache confirms then saves enabled=false', async () => {
    mockGetConfig.mockResolvedValue(CONFIGURED_ENABLED_CONFIG);
    const user = userEvent.setup();
    renderWithProviders(<CachePage />);
    await user.click(await screen.findByRole('button', { name: /disable caches/i }));
    await user.click(await screen.findByText(i18n.t('pages:aiGateway.cache.statusStrip.disableSemanticItem')));
    await user.click(await screen.findByRole('button', { name: i18n.t('pages:aiGateway.cache.statusStrip.confirmButton') }));
    await waitFor(() => expect(mockSaveConfig).toHaveBeenCalledWith(expect.objectContaining({ enabled: false })));
  });
});
...
describe('CachePage — embedding + semantic form', () => {
  ...
  it('clicking Run Embedding Probe shows the probe latency result', async () => {
    mockGetConfig.mockResolvedValue(CONFIGURED_ENABLED_CONFIG);
    renderWithProviders(<CachePage />);
    const probeBtn = await screen.findByRole('button', { name: /run embedding probe/i });
    await userEvent.click(probeBtn);
    // mockRunProbe → latencyMs 42 (fast tier)
    await waitFor(() => expect(mockRunProbe).toHaveBeenCalledWith('prov-1'));
    expect(await screen.findByText(/^fast$/i)).toBeInTheDocument();
  });

  it('opening Pre-warm Corpus shows the import modal', async () => {
    mockGetConfig.mockResolvedValue(CONFIGURED_ENABLED_CONFIG);
    renderWithProviders(<CachePage />);
    await userEvent.click(await screen.findByRole('button', { name: /pre-warm corpus/i }));
    expect(await screen.findByText(/import pre-warm corpus/i)).toBeInTheDocument();
  });
});
...
describe('CachePage — Freshness rules card', () => {
  ...
  it('Add Rule → fill id + keywords → Save creates the pattern', async () => {
    renderWithProviders(<CachePage />);
    await waitFor(() => expect(screen.getByRole('heading', { name: /freshness rules/i, level: 3 })).toBeDefined());
    await userEvent.click(screen.getByRole('button', { name: /add rule/i }));
    const dialog = await screen.findByRole('dialog');
    fireEvent.change(within(dialog).getByPlaceholderText('my-custom-rule'), { target: { value: 'sports-scores' } });
    fireEvent.change(within(dialog).getByPlaceholderText('trending,viral,breaking'), { target: { value: 'score, fixture' } });
    fireEvent.click(within(dialog).getByRole('button', { name: /^save$/i }));
    await waitFor(() => expect(mockCreatePattern).toHaveBeenCalledWith(expect.objectContaining({ id: 'sports-scores', keywords: ['score', 'fixture'] })));
  });

  it('deleting the seed rule confirms then calls delete', async () => {
    renderWithProviders(<CachePage />);
    await waitFor(() => expect(screen.getAllByText('weather').length).toBeGreaterThanOrEqual(1));
    await userEvent.click(screen.getByRole('button', { name: i18n.t('pages:aiGateway.cache.freshness.deleteAriaLabel', { id: 'weather' }) }));
    const dialog = await screen.findByRole('alertdialog');
    await userEvent.click(within(dialog).getByRole('button', { name: i18n.t('common:delete') }));
    await waitFor(() => expect(mockDeletePattern).toHaveBeenCalledWith('weather'));
  });
});
