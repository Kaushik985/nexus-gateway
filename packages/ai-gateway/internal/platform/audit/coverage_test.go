...
func TestNormalizeAdapterType_LowercasesAndPassesThrough(t *testing.T) {
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			// Request direction always uses IngressFormat (lower-cased).
			got := normalizeAdapterType(&Record{IngressFormat: tc.in}, "request")
			if got != tc.want {
				t.Errorf("normalizeAdapterType(req,%q) = %q, want %q", tc.in, got, tc.want)
			}
		})
	}
}

// TestNormalizeAdapterType_ResponseDirectionPrefersUpstream verifies the
// cross-format fix: when the routed target's adapter type is recorded,
// the response-side normalizer is selected by that key (not the ingress
// format). Required because cross-format requests like /v1/responses →
// Anthropic capture upstream-shaped bytes; using the ingress key would
// feed them to the wrong unmarshaler.
func TestNormalizeAdapterType_ResponseDirectionPrefersUpstream(t *testing.T) {
	r := &Record{IngressFormat: "openai-responses", UpstreamAdapterType: "anthropic"}
	if got := normalizeAdapterType(r, "response"); got != "anthropic" {
		t.Errorf("response normalizer key = %q, want anthropic", got)
	}
	if got := normalizeAdapterType(r, "request"); got != "openai-responses" {
		t.Errorf("request normalizer key = %q, want openai-responses", got)
	}
	// Falls back to IngressFormat on response when UpstreamAdapterType is
	// empty (e.g. early failure before routing).
	r2 := &Record{IngressFormat: "openai", UpstreamAdapterType: ""}
	if got := normalizeAdapterType(r2, "response"); got != "openai" {
		t.Errorf("response normalizer fallback = %q, want openai", got)
	}
}
...
// applyStorageAction — covers every branch.
...
func TestApplyStorageAction_EmptyInput(t *testing.T) {
	...
	for _, action := range []string{"", "keep", "redact", "drop-content"} {
		got := applyStorageAction(nil, action, nil, nil)
		if got != nil {
...
func TestApplyStorageAction_KeepNoop(t *testing.T) {
	...
	for _, action := range []string{"", "keep"} {
		got := applyStorageAction(raw, action, nil, nil)
...
func TestApplyStorageAction_UnknownActionFallsThrough(t *testing.T) {
	raw := json.RawMessage(`{"kind":"ai-chat"}`)
	got := applyStorageAction(raw, "unrecognized", nil, nil)
	if string(got) != string(raw) {
		t.Errorf("unknown action → want unchanged, got %q", got)
	}
}

func TestApplyStorageAction_RedactNoSpansReturnsRaw(t *testing.T) {
	raw := json.RawMessage(`{"kind":"ai-chat","messages":[]}`)
	got := applyStorageAction(raw, "redact", nil, nil)
	if string(got) != string(raw) {
		t.Errorf("redact with empty spans → want unchanged, got %q", got)
	}
}

func TestApplyStorageAction_RedactBadJSONFallsBack(t *testing.T) {
	raw := json.RawMessage(`not-json`)
	spans := []normalize.TransformSpan{{ContentAddress: "messages.0.content.0", Start: 0, End: 3}}
	got := applyStorageAction(raw, "redact", spans, nil)
	if string(got) != string(raw) {
		t.Errorf("redact with un-parsable JSON → want fallback to raw, got %q", got)
	}
}

func TestApplyStorageAction_RedactAppliesSpans(t *testing.T) {
	// Build a NormalizedPayload via the package to ensure shapes match.
	...
	// Verify the redacted text landed inside the content slot.
...
func TestApplyStorageAction_DropContentPlaceholder(t *testing.T) {
	// Valid input — placeholder preserves Kind/NormalizeVersion/Protocol from source.
	p := normalize.NormalizedPayload{
		Kind:             normalize.KindAIChat,
	...
	got := applyStorageAction(raw, "drop-content", nil, ruleIDs)
	var decoded normalize.NormalizedPayload
	if err := json.Unmarshal(got, &decoded); err != nil {
		t.Fatalf("unmarshal: %v", err)
	}
...
func TestApplyStorageAction_DropContent_BadJSONStillEmitsPlaceholder(t *testing.T) {
	...
	got := applyStorageAction(raw, "drop-content", nil, []string{"r1"})
	var decoded normalize.NormalizedPayload
	if err := json.Unmarshal(got, &decoded); err != nil {
	...
	// Defaults applied because Unmarshal failed and original Kind/Version weren't read.
	if decoded.Kind != normalize.KindAIChat {
...
func TestRecordToMessage_CacheZeroFieldsStayNil(t *testing.T) {
	...
	if msg.NormalizedStripCount != nil {
		t.Error("zero NormalizedStripCount should not be stamped")
	}
	if msg.NormalizedStripBytes != nil {
		t.Error("zero NormalizedStripBytes should not be stamped")
	}
