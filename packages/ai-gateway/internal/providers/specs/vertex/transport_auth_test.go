...
// TestApplyAuth_ServiceAccountJSON_ConcurrentMintCollapses proves the
// per-key mint serialization: N concurrent requests for the same service
// account on a cold cache collapse to a SINGLE OAuth round-trip instead of
// a thundering herd. The token endpoint is gated so all callers pile up on
// the mint before any completes.
func TestApplyAuth_ServiceAccountJSON_ConcurrentMintCollapses(t *testing.T) {
	gate := make(chan struct{})
	var calls int
	var mu sync.Mutex
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		<-gate // block until released so callers overlap on the mint
		mu.Lock()
		calls++
		mu.Unlock()
		w.Header().Set("Content-Type", "application/json")
	...
	for range n {
		...
		go func() {
			...
			if req.Header.Get("Authorization") != "Bearer minted" {
				t.Errorf("Authorization=%q", req.Header.Get("Authorization"))
			}
		}()
	}
	time.Sleep(30 * time.Millisecond) // let goroutines pile up on the mint lock
	close(gate)
	wg.Wait()
	...
	defer mu.Unlock()
	if calls != 1 {
		t.Fatalf("token endpoint hit %d times for %d concurrent requests, want 1 (herd not collapsed)", calls, n)
	}
}
