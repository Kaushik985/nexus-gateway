...
func TestAnthropic_StreamResponseMalformedEventStillYieldsPartial(t *testing.T) {
	// Inject a non-JSON data line — partial parse with lastErr set.
	...
	if err == nil {
		t.Fatal("expected partial-parse err")
	}
...
func TestOpenAIChat_StreamResponse_BadJSONLineLastErr(t *testing.T) {
	raw := strings.Join([]string{
	...
	if len(got.Messages) == 0 || got.Messages[0].Content[0].Text != "a" {
		t.Fatalf("partial parse should preserve usable text; got %+v", got.Messages)
	}
}
...
func TestGemini_StreamMalformedDataLine(t *testing.T) {
	...
	if got.Messages[0].Content[0].Text != "ok" {
		t.Fatalf("partial parse should preserve text: %+v", got.Messages)
	}
}
