...
type cost struct {
	...
	cf        kit.Confirm // prod-gated cache flush
	flushing  bool
	flushNote string
	flushErr  error
}
...
func newCost(gw kit.Gateway, s ...kit.Session) *cost {
	return &cost{gw: gw, loading: true, cf: kit.NewConfirm(kit.OptSession(s))}
}
...
func (c *cost) Help() string {
	...
	return "f flush gateway cache · 1-9 jump · / commands · tab chat · q quit"
}
...
func (c *cost) Update(msg tea.Msg) (kit.ViewModel, tea.Cmd) {
	switch msg := msg.(type) {
	case costMsg:
		c.loading = false
		c.err = msg.err
		...
		return c, kit.Tick(kit.PollSlow, costTick{})
	case costTick:
		return c, c.fetch()
	case cacheFlushMsg:
		c.flushing = false
...
func (c *cost) providerTable() string {
	...
	for _, r := range c.byProv.Data {
		...
		lines = append(lines, fmt.Sprintf("  %-22s %8d %12d %10s %10s",
			label, r.RequestCount, r.TotalTokens, fmt.Sprintf("$%.4f", r.TotalEstCostUSD), kit.Ms(int(r.AvgLatencyMs))))
