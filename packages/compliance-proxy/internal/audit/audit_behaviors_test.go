...
// branch: the spool is pre-seeded past its 1 MB quota so the NDJSON write is
// refused, and fallbackToNDJSON must log (not panic).
func TestFallbackToNDJSON_NDJSONWriteError(t *testing.T) {
	dir := t.TempDir()
	ndjson, err := NewNDJSONWriter(dir, "inst-fb-err", 10, 1, quietLogger())
	if err != nil {
		t.Fatalf("NewNDJSONWriter: %v", err)
	}
	defer ndjson.Close() //nolint:errcheck
	...
	defer w.Close(context.Background()) //nolint:errcheck
	// Seed the spool past its 1 MB quota so the next NDJSON write is refused
	// with quota-exceeded, exercising the fallback's write-error path.
	seedPath := filepath.Join(dir, "inst-fb-err", "preexisting.ndjson")
	if err := os.WriteFile(seedPath, make([]byte, 1100*1024), 0600); err != nil {
		t.Fatalf("seed: %v", err)
	}
...
// Rotation-with-no-open-file is now exercised in the shared spill writer's
// tests (packages/shared/audit/ndjson); the proxy wrapper only delegates.
...
// overflow default arm: when w.ndjson.Write returns an error, the error must
// be logged (not panic). We force the failure by seeding the spool past its
// 1 MB quota so the next NDJSON write is refused.
func TestEnqueue_OverflowWriteErrorLogs(t *testing.T) {
	dir := t.TempDir()
	ndjson, err := NewNDJSONWriter(dir, "inst-overflow-werr", 10, 1, quietLogger())
	if err != nil {
		t.Fatalf("NewNDJSONWriter: %v", err)
	}
	defer ndjson.Close() //nolint:errcheck
	seed := filepath.Join(dir, "inst-overflow-werr", "preexisting.ndjson")
	if err := os.WriteFile(seed, make([]byte, 1100*1024), 0600); err != nil {
		t.Fatalf("seed: %v", err)
	}
