...
func TestDecodeResponse_NonChatPassthroughAndEmpty(t *testing.T) {
	body := []byte(`{"data":[]}`)
	decRes, err := codec{}.DecodeResponse(typology.WireShapeNone, body, "")
	...
	decRes, err = codec{}.DecodeResponse(typology.WireShapeAnthropicMessages, nil, "")
	out = decRes.CanonicalBody
	if err != nil {
		t.Fatal(err)
	}
...
func TestErrorNormalizer_TypeMatrix(t *testing.T) {
	cases := []struct {
		...
		{"overloaded_error", 503, provcore.CodeUpstreamError},
...
func TestMapAnthropicStreamError_AllBranches(t *testing.T) {
	cases := []struct {
		...
		{"not_found_error", provcore.CodeUpstreamError, http.StatusNotFound, "x", false},
