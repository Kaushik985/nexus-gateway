...
// openai_chat.go — normalizeStreamResponse partial-parse error propagation

// TestOpenAIChat_StreamResponsePartialParseErrorSurfaces covers lines 613-615
// and 663-664: a stream that contains valid chunks followed by a malformed JSON
// chunk returns both the accumulated messages AND an error (partial-parse
// failure mode), allowing callers to decide whether to surface partial results.
func TestOpenAIChat_StreamResponsePartialParseErrorSurfaces(t *testing.T) {
	...
	// The stream saw a valid first chunk so sawAny=true — error should be
	// a partial-parse wrapped error, not ErrUnsupported.
	if err == nil {
		t.Fatal("expected partial-parse error for malformed chunk, got nil")
	}
	if errors.Is(err, core.ErrUnsupported) {
		t.Errorf("partial-parse error should not be ErrUnsupported: %v", err)
	}
...
// TestGenericHTTP_JSONContentTypeWithSSEFallback covers lines 169-171:
// a body stamped as application/json that looks like an SSE stream (starts
// with "data: ") is re-routed to the SSE projection so the operator sees the
// event stream verbatim instead of a JSON decode error.
func TestGenericHTTP_JSONContentTypeWithSSEFallback(t *testing.T) {
	...
	// SSE fallback emits KindHTTPText.
	if got.Kind != core.KindHTTPText {
		t.Errorf("Kind = %v, want http-text for SSE mislabelled as JSON", got.Kind)
	}
}
