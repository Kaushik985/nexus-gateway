...
func TestExtractRequest_OpenAICompatDelegation(t *testing.T) {
	body := []byte(`{
		"model":"sonar-pro",
		"messages":[{"role":"user","content":"hi from perplexity"}]
	}`)
	a := &Adapter{}
	...
	if len(nc.Segments) != 1 || nc.Segments[0] != "hi from perplexity" {
...
func TestDetectRequestMeta_PerplexityKeyClass(t *testing.T) {
	body := []byte(`{"messages":[{"role":"user","content":"hi"}]}`)
	a := &Adapter{}
	r, _ := http.NewRequest(http.MethodPost, "https://api.perplexity.ai/chat/completions", nil)
	r.Header.Set("Authorization", "Bearer pplx-abcdef123456")
	meta := a.DetectRequestMeta(r, body)
	if meta.Provider != "perplexity" {
		t.Errorf("Provider=%q", meta.Provider)
	}
	if meta.ApiKeyClass != "perplexity-bearer" {
		t.Errorf("ApiKeyClass=%q", meta.ApiKeyClass)
	}
}

// Configure is a no-op for perplexity (delegates to openai inner which is also
...
// surface end-to-end through the perplexity wrapper.
func TestExtractResponse_ChatCompletionsDelegation(t *testing.T) {
	...
	if len(nc.Segments) != 1 || nc.Segments[0] != "hello from perplexity" {
		t.Errorf("Segments=%v", nc.Segments)
	}
...
func TestExtractResponse_Malformed(t *testing.T) {
	a := &Adapter{}
	_, err := a.ExtractResponse(context.Background(), []byte(`not json`), "/chat/completions")
...
// delta must produce a single Segment. The perplexity wrapper does no
// transform here.
...
func TestExtractStreamChunk_Malformed(t *testing.T) {
	a := &Adapter{}
	_, err := a.ExtractStreamChunk(context.Background(), []byte(`not json`), "/chat/completions")
	if !errors.Is(err, traffic.ErrMalformed) {
		t.Errorf("err=%v want ErrMalformed", err)
	}
}
...
// rewriter and substitutes per-message text — pin that the perplexity
...
func TestRewriteRequestBody_ChatCompletionsDelegation(t *testing.T) {
	...
	a := &Adapter{}
	out, n, err := a.RewriteRequestBody(context.Background(), body, "/chat/completions", traffic.NormalizedContent{
...
// perplexity wrapper preserves the rewritten output and patch count.
func TestRewriteResponseBody_ChatCompletionsDelegation(t *testing.T) {
	...
	a := &Adapter{}
	out, n, err := a.RewriteResponseBody(context.Background(), body, "/chat/completions", traffic.NormalizedContent{
		Segments: []string{"REDACTED"},
	})
	if err != nil {
		t.Fatalf("err=%v", err)
...
// confirms the wrapper does not invent a stale "perplexity-bearer" tag.
func TestDetectRequestMeta_NoAuth(t *testing.T) {
	body := []byte(`{"model":"sonar-pro","messages":[{"role":"user","content":"hi"}]}`)
	a := &Adapter{}
	r, _ := http.NewRequest(http.MethodPost, "https://api.perplexity.ai/chat/completions", nil)
	meta := a.DetectRequestMeta(r, body)
	if meta.ApiKeyClass != "" {
...
// perplexity wrapper passes through unchanged, so prompt/completion split
...
// Normalize: perplexity speaks the openai-chat wire shape. A canonical
// chat-completions body must claim Tier-1 with DetectedSpec=perplexity
