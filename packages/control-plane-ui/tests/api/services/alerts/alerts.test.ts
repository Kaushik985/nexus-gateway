...
import { alertsApi } from '../../../../src/api/services/alerts/alerts';
...
describe('alertsApi', () => {
  ...
  describe('rules', () => {
    ...
    it('listRules() with no params omits the query string', async () => {
      await alertsApi.listRules();
      expect(getSpy).toHaveBeenCalledWith('/api/admin/alerts/rules');
    });
    ...
    it('getRule + resetRule encode the rule id', async () => {
      await alertsApi.getRule('quota.threshold');
      await alertsApi.resetRule('quota.threshold');
      expect(getSpy).toHaveBeenCalledWith('/api/admin/alerts/rules/quota.threshold');
      expect(postSpy).toHaveBeenCalledWith('/api/admin/alerts/rules/quota.threshold/reset', {});
    });
  });
  ...
  describe('channels', () => {
    ...
    it('channel CRUD routes to /api/admin/alerts/channels', async () => {
      await alertsApi.listChannels();
      await alertsApi.getChannel('c-1');
      await alertsApi.createChannel({ name: 'slack', type: 'webhook', config: {} } as never);
      await alertsApi.updateChannel('c-1', { name: 'slack-2' });
      await alertsApi.deleteChannel('c-1');
      expect(getSpy).toHaveBeenCalledWith('/api/admin/alerts/channels');
      expect(getSpy).toHaveBeenCalledWith('/api/admin/alerts/channels/c-1');
      expect(postSpy).toHaveBeenCalledWith('/api/admin/alerts/channels', { name: 'slack', type: 'webhook', config: {} });
      expect(putSpy).toHaveBeenCalledWith('/api/admin/alerts/channels/c-1', { name: 'slack-2' });
      expect(deleteSpy).toHaveBeenCalledWith('/api/admin/alerts/channels/c-1');
    });
  });
});
