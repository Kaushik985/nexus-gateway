...
func TestStore_ResolveByRuleTargetIfAcknowledged_Mock(t *testing.T) {
	t.Run("happy returns count of acknowledged rows resolved", func(t *testing.T) {
		mock, _ := pgxmock.NewPool()
		defer mock.Close()
		// SQL must target ACKNOWLEDGED-only — assert that in the matcher.
		mock.ExpectExec(`UPDATE "Alert".*ACKNOWLEDGED`).WithArgs("r", "t", pgxmock.AnyArg(), "reason").
			WillReturnResult(pgconn.NewCommandTag("UPDATE 2"))
		s := NewStoreWithPgxPool(mock)
		n, err := s.ResolveByRuleTargetIfAcknowledged(context.Background(), "r", "t", "reason")
		if err != nil || n != 2 {
			t.Fatalf("n=%d err=%v", n, err)
		}
	})
	t.Run("error wraps", func(t *testing.T) {
		...
		mock.ExpectExec(`UPDATE "Alert"`).WithArgs(anyN(4)...).
			WillReturnError(errors.New("boom"))
		s := NewStoreWithPgxPool(mock)
		_, err := s.ResolveByRuleTargetIfAcknowledged(context.Background(), "r", "t", "x")
		if err == nil || !strings.Contains(err.Error(), "acknowledged only") {
			t.Fatalf("want wrap: %v", err)
		}
	})
}

func TestStore_CountFiringByRuleTarget_Mock(t *testing.T) {
	t.Run("happy returns count", func(t *testing.T) {
		mock, _ := pgxmock.NewPool()
		defer mock.Close()
		mock.ExpectQuery(`SELECT COUNT\(\*\) FROM "Alert"`).WithArgs("r", "t").
			WillReturnRows(pgxmock.NewRows([]string{"count"}).AddRow(3))
		s := NewStoreWithPgxPool(mock)
		n, err := s.CountFiringByRuleTarget(context.Background(), "r", "t")
		if err != nil || n != 3 {
			t.Fatalf("n=%d err=%v", n, err)
		}
	})
	t.Run("error wraps", func(t *testing.T) {
		...
		mock.ExpectQuery(`SELECT COUNT\(\*\) FROM "Alert"`).WithArgs("r", "t").
			WillReturnError(errors.New("boom"))
		s := NewStoreWithPgxPool(mock)
		_, err := s.CountFiringByRuleTarget(context.Background(), "r", "t")
		if err == nil || !strings.Contains(err.Error(), "count firing by rule+target") {
			t.Fatalf("want wrap: %v", err)
		}
	})
}

func TestStore_CountChannels_Mock(t *testing.T) {
	t.Run("happy returns count", func(t *testing.T) {
		mock, _ := pgxmock.NewPool()
		defer mock.Close()
		mock.ExpectQuery(`SELECT COUNT\(\*\) FROM "AlertChannel"`).
			WillReturnRows(pgxmock.NewRows([]string{"count"}).AddRow(5))
		s := NewStoreWithPgxPool(mock)
		n, err := s.CountChannels(context.Background())
		if err != nil || n != 5 {
			t.Fatalf("n=%d err=%v", n, err)
		}
	})
	...
	t.Run("error wraps", func(t *testing.T) {
		...
		mock.ExpectQuery(`SELECT COUNT\(\*\) FROM "AlertChannel"`).
			WillReturnError(errors.New("boom"))
		s := NewStoreWithPgxPool(mock)
		_, err := s.CountChannels(context.Background())
		if err == nil || !strings.Contains(err.Error(), "count channels") {
			t.Fatalf("want wrap: %v", err)
		}
	})
}
...
// expectGetRuleAck queues a GetRule call returning a rule with the given
// requiresAck value (and requiresAck-relevant defaults otherwise).
func expectGetRuleAck(mock pgxmock.PgxPoolIface, id string, requiresAck bool) {
	row := makeRuleRow(id, true) // enabled=true
	row[4] = requiresAck         // requiresAck column
	mock.ExpectQuery(`FROM "AlertRule".*WHERE id`).WithArgs(id).
		WillReturnRows(pgxmock.NewRows(ruleRowCols()).AddRow(row...))
}

func TestRaiser_Resolve_Mock(t *testing.T) {
	// requiresAck=false (default) resolves BOTH FIRING and ACKNOWLEDGED via
	// ResolveByRuleTarget. Each case queues GetRule (requiresAck=false) then
	// the single UPDATE "Alert" exec.
	t.Run("requiresAck=false happy with rows", func(t *testing.T) {
		mock, _ := pgxmock.NewPool()
		defer mock.Close()
		expectGetRuleAck(mock, "r", false)
		mock.ExpectExec(`UPDATE "Alert"`).WithArgs(anyN(4)...).
			WillReturnResult(pgconn.NewCommandTag("UPDATE 2"))
		s := NewStoreWithPgxPool(mock)
		r := NewRaiserWithPool(mock, s, nil, quietLogger())
	...
	t.Run("requiresAck=false zero rows (no log)", func(t *testing.T) {
		mock, _ := pgxmock.NewPool()
		defer mock.Close()
		expectGetRuleAck(mock, "r", false)
		mock.ExpectExec(`UPDATE "Alert"`).WithArgs(anyN(4)...).
	...
	t.Run("requiresAck=false nil logger still works", func(t *testing.T) {
		mock, _ := pgxmock.NewPool()
		defer mock.Close()
		expectGetRuleAck(mock, "r", false)
		mock.ExpectExec(`UPDATE "Alert"`).WithArgs(anyN(4)...).
	...
	t.Run("requiresAck=false store error wraps", func(t *testing.T) {
		mock, _ := pgxmock.NewPool()
		defer mock.Close()
		expectGetRuleAck(mock, "r", false)
		mock.ExpectExec(`UPDATE "Alert"`).WithArgs(anyN(4)...).
	...
	// requiresAck=true resolves ONLY ACKNOWLEDGED rows. The Raiser first counts
	// FIRING rows (for the skip log), then runs the acknowledged-only UPDATE.
	t.Run("requiresAck=true resolves acknowledged, skips firing", func(t *testing.T) {
		mock, _ := pgxmock.NewPool()
		defer mock.Close()
		expectGetRuleAck(mock, "r", true)
		// CountFiringByRuleTarget: 1 firing row will be skipped.
		mock.ExpectQuery(`SELECT COUNT\(\*\) FROM "Alert"`).WithArgs("r", "t").
			WillReturnRows(pgxmock.NewRows([]string{"count"}).AddRow(1))
		// Acknowledged-only UPDATE resolves 1 row.
		mock.ExpectExec(`UPDATE "Alert".*ACKNOWLEDGED`).WithArgs(anyN(4)...).
			WillReturnResult(pgconn.NewCommandTag("UPDATE 1"))
		s := NewStoreWithPgxPool(mock)
		r := NewRaiserWithPool(mock, s, nil, quietLogger())
		if err := r.Resolve(context.Background(), "r", "t", "reason"); err != nil {
	...
	t.Run("requiresAck=true count error wraps", func(t *testing.T) {
		mock, _ := pgxmock.NewPool()
		defer mock.Close()
		expectGetRuleAck(mock, "r", true)
		mock.ExpectQuery(`SELECT COUNT\(\*\) FROM "Alert"`).WithArgs("r", "t").
	...
	t.Run("requiresAck=true resolve error wraps", func(t *testing.T) {
		mock, _ := pgxmock.NewPool()
		defer mock.Close()
		expectGetRuleAck(mock, "r", true)
		mock.ExpectQuery(`SELECT COUNT\(\*\) FROM "Alert"`).WithArgs("r", "t").
	...
	t.Run("requiresAck=true nil logger skips logging", func(t *testing.T) {
		mock, _ := pgxmock.NewPool()
		defer mock.Close()
		expectGetRuleAck(mock, "r", true)
		mock.ExpectQuery(`SELECT COUNT\(\*\) FROM "Alert"`).WithArgs("r", "t").
	...
	t.Run("rule not found falls back to resolve-both", func(t *testing.T) {
		mock, _ := pgxmock.NewPool()
		defer mock.Close()
		// GetRule returns no rows -> ErrNotFound -> resolve-both fallback.
		mock.ExpectQuery(`FROM "AlertRule".*WHERE id`).WithArgs("missing").
			WillReturnRows(pgxmock.NewRows(ruleRowCols()))
		mock.ExpectExec(`UPDATE "Alert"`).WithArgs(anyN(4)...).
			WillReturnResult(pgconn.NewCommandTag("UPDATE 2"))
		s := NewStoreWithPgxPool(mock)
		r := NewRaiserWithPool(mock, s, nil, quietLogger())
		if err := r.Resolve(context.Background(), "missing", "t", "reason"); err != nil {
			t.Fatal(err)
		}
		if err := mock.ExpectationsWereMet(); err != nil {
			t.Fatalf("unmet expectations: %v", err)
	...
	t.Run("rule not found nil logger fallback", func(t *testing.T) {
		mock, _ := pgxmock.NewPool()
		defer mock.Close()
		mock.ExpectQuery(`FROM "AlertRule".*WHERE id`).WithArgs("missing").
			WillReturnRows(pgxmock.NewRows(ruleRowCols()))
	...
	t.Run("rule load error wraps", func(t *testing.T) {
		mock, _ := pgxmock.NewPool()
		defer mock.Close()
		mock.ExpectQuery(`FROM "AlertRule".*WHERE id`).WithArgs("r").
			WillReturnError(errors.New("boom"))
		s := NewStoreWithPgxPool(mock)
		r := NewRaiserWithPool(mock, s, nil, quietLogger())
		err := r.Resolve(context.Background(), "r", "t", "x")
...
type adminFakeStore struct {
	...
	countChannelsFn     func(ctx context.Context) (int, error)
...
func TestAdminUpdateRule(t *testing.T) {
	...
	enabledRefresh := func() func(context.Context, string) (*AlertRule, error) {
		first := true
		return func(_ context.Context, id string) (*AlertRule, error) {
			if first {
				first = false
				return baseGet(id), nil // pre-update read
			}
			r := baseGet(id)
			r.Enabled = true // refreshed: rule is now enabled
			return r, nil
		}
	}
	t.Run("enable with zero channels -> 200 with warning", func(t *testing.T) {
		store := &adminFakeStore{
			getRuleFn:       enabledRefresh(),
			updateRuleFn:    func(context.Context, AlertRule) error { return nil },
			countChannelsFn: func(context.Context) (int, error) { return 0, nil },
		}
		h := AdminHandlers{Store: store}
		...
		if len(resp.Warnings) != 1 || resp.Warnings[0] != "no_delivery_channel_configured" {
			t.Errorf("want [no_delivery_channel_configured], got %v", resp.Warnings)
		}
		if !resp.Enabled {
			t.Errorf("refreshed rule should be enabled")
		}
	})
	t.Run("enable with channels present -> 200 no warning", func(t *testing.T) {
		store := &adminFakeStore{
			getRuleFn:       enabledRefresh(),
			updateRuleFn:    func(context.Context, AlertRule) error { return nil },
			countChannelsFn: func(context.Context) (int, error) { return 2, nil },
		}
		h := AdminHandlers{Store: store}
		c, rec := makeEchoRequest(http.MethodPut, "/", `{"enabled":true}`)
		setParam(c, "id", "r.1")
		...
		// warnings must be present-but-empty (never null), and never count
		// channels into a warning when at least one exists.
		if !strings.Contains(rec.Body.String(), `"warnings":[]`) {
			t.Errorf("want empty warnings array, got %s", rec.Body.String())
		}
	})
	t.Run("disable never warns (CountChannels not consulted)", func(t *testing.T) {
		store := &adminFakeStore{
			getRuleFn:    func(_ context.Context, id string) (*AlertRule, error) { return baseGet(id), nil }, // Enabled=false
			updateRuleFn: func(context.Context, AlertRule) error { return nil },
			countChannelsFn: func(context.Context) (int, error) {
				t.Error("CountChannels must not be called for a disabled rule")
				return 0, nil
			},
		}
		h := AdminHandlers{Store: store}
		c, rec := makeEchoRequest(http.MethodPut, "/", `{"enabled":false}`)
		setParam(c, "id", "r.1")
		_ = h.UpdateRule(c)
		if rec.Code != http.StatusOK {
			t.Fatalf("code=%d", rec.Code)
	...
	t.Run("CountChannels error -> 500", func(t *testing.T) {
		store := &adminFakeStore{
			getRuleFn:       enabledRefresh(),
			updateRuleFn:    func(context.Context, AlertRule) error { return nil },
			countChannelsFn: func(context.Context) (int, error) { return 0, errors.New("boom") },
		}
		h := AdminHandlers{Store: store}
		c, rec := makeEchoRequest(http.MethodPut, "/", `{"enabled":true}`)
		setParam(c, "id", "r.1")
