...
// DetectRequestMeta delegates then re-labels Provider as "mistral".
// Mistral keys are typically opaque base62 strings prefixed by Bearer;
// classify them under the "mistral-bearer" key class.
func (a *Adapter) DetectRequestMeta(r *http.Request, body []byte) traffic.RequestMeta {
	...
	if r != nil {
		...
		if strings.HasPrefix(auth, "Bearer ") {
			...
			if tok != "" {
				meta.ApiKeyClass = "mistral-bearer"
