...
import {
  quotaOverrideApi, organizationApi, projectApi, virtualKeyApi, iamApi,
} from '@/api/services';
import type { CreateQuotaOverrideInput, QuotaOverride } from '@/api/services';
...
/* -- Target fetcher (plain function, not a hook) -------------------------- */
...
async function fetchTargetOptions(targetType: string, query: string): Promise<ComboboxOption[]> {
  switch (targetType) {
    ...
    case 'organization': {
      ...
      const rows = res.data ?? [];
      return rows.slice(0, 100).map((o) => ({ id: o.id, label: `${o.name} (${o.code})` }));
    }
    default:
      return [];
  }
...
export function QuotaOverrideEdit() {
  const { id } = useParams<{ id: string }>();

  const { data: override, loading: fetching, error } = useApi<QuotaOverride>(
    () => quotaOverrideApi.get(id!),
    ['admin', 'quota-overrides', id!],
  );

  if (fetching || !override) return <Skeleton.ListPageSkeleton />;
  if (error) return <ErrorBanner message={error.message} />;

  return <QuotaOverrideEditForm id={id!} override={override} />;
}

function QuotaOverrideEditForm({ id, override }: { id: string; override: QuotaOverride }) {
  const { t } = useTranslation();
  const navigate = useNavigate();

  const [targetLabel, setTargetLabel] = useState(override.targetName ?? override.targetId);
  ...
  const form = useZodForm({
    ...
    defaultValues: {
      ...
      costLimitUsd: override.costLimitUsd != null ? String(override.costLimitUsd) : '',
      tokenLimit: override.tokenLimit != null ? String(override.tokenLimit) : '',
      enforcementMode: override.enforcementMode ?? '_inherit',
      periodType: override.periodType ?? '_inherit',
    },
  });
  ...
  return (
    <Stack gap="md">
      <Breadcrumb items={[
        ...
        { label: t('pages:quotaOverrides.editOverride') },
      ]} />

      <PageHeader
        title={t('pages:quotaOverrides.editOverride')}
        subtitle={t('pages:quotaOverrides.editSubtitle')}
      />
