...
func TestResolve_NoSelection(t *testing.T) {
	c := &Config{Envs: map[string]Env{}} // no default, no envs
	_, err := c.Resolve("", "")
	if err == nil {
...
func TestResolve_FillsNameFromKey(t *testing.T) {
	c := &Config{DefaultEnv: "x", Envs: map[string]Env{"x": {CPBaseURL: "u"}}} // Env.Name empty
	got, err := c.Resolve("", "")
...
func TestSaveLoad_RoundTrip(t *testing.T) {
	...
	c := sampleConfig(path)
	c.Envs["prod"] = Env{Name: "prod", CPBaseURL: "https://cp.example.com", LastModel: "gpt-4", LastVKName: "research"}
...
func TestSetEnvAndSetDefault(t *testing.T) {
	c := &Config{Envs: nil}
	c.SetEnv(Env{Name: "dev", CPBaseURL: "u"})
...
func TestRemoveEnv(t *testing.T) {
	c := &Config{DefaultEnv: "dev", Envs: map[string]Env{"dev": {Name: "dev"}, "local": builtinLocalEnv()}}
...
func TestConfig_PersistsNoSecrets(t *testing.T) {
	...
	// token even after we store secrets out-of-band. Prove it: store secrets,
	// save config, assert the file bytes contain none of them.
	path := filepath.Join(t.TempDir(), "config.toml")
	store := newMemStore()
	_ = store.Set("local", SecretAccessToken, "SECRET-ACCESS-TOKEN-XYZ")
	_ = store.Set("local", SecretAdminKey, "nxk_SECRET_ADMIN_KEY")
	c := sampleConfig(path)
	if err := c.Save(); err != nil {
		t.Fatalf("save: %v", err)
	}
...
func TestErrSecretNotFoundIsStable(t *testing.T) {
	// Guard: memStore.Get must report the shared sentinel.
