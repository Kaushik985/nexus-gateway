...
// consumer types (TrafficEventWriter, AdminAuditWriter) without
...
func TestStripNulJSON_PreservesCleanAndStripsTainted(t *testing.T) {
	...
	escaped := json.RawMessage(`{"k":"a\u0000b"}`)
	if got := string(stripNulJSON(escaped)); got != `{"k":"ab"}` {
		t.Errorf("escaped \\u0000: got %q, want %q", got, `{"k":"ab"}`)
	}
	// Both forms present in one payload — strip each.
	both := json.RawMessage("{\"k\":\"a\x00b\\u0000c\"}")
	if got := string(stripNulJSON(both)); got != `{"k":"abc"}` {
		t.Errorf("both forms: got %q, want %q", got, `{"k":"abc"}`)
	}
	// A payload that contains the escape but no raw byte still triggers the
	// strip path (exercises the hasEscaped-only branch).
	onlyEscaped := json.RawMessage(`["\u0000"]`)
	if got := string(stripNulJSON(onlyEscaped)); got != `[""]` {
		t.Errorf("only-escaped: got %q, want %q", got, `[""]`)
	}
}
...
//
// Observable behavior: with a closed pool, Begin fails for both the batch
// fast path and the per-item fallback, so flush:
...
func TestTrafficWriter_InsertNormalizedPayloads_WriteWhenPresent_DefaultsVersion(t *testing.T) {
	...
	defer mock.Close()
	ctx := context.Background()
	// Fast path: outer Begin, SAVEPOINT (Begin) → batch → RELEASE (Commit),
	// then the outer Commit.
	mock.ExpectBegin()
	mock.ExpectBegin()
	ebN := mock.ExpectBatch()
	ebN.ExpectExec(`INSERT INTO traffic_event_normalized`).WithArgs(anyArgs(10)...).WillReturnResult(pgconn.NewCommandTag("INSERT 0 1"))
	mock.ExpectCommit()
	mock.ExpectCommit()
...
// A non-poison sidecar error: the fast-path batch fails and is rolled back to
// the savepoint, then the row-by-row retry also fails (non-poison). The error
// is returned (so the caller logs + counts it) but the savepoints isolate the
// failure so the outer tx — and the raw rows — stay committable. The per-row
// savepoint Rollback is also made to fail here, exercising that warn branch.
func TestTrafficWriter_InsertNormalizedPayloads_RowErrorPropagatesButRollsOnlySavepoint(t *testing.T) {
	mock, err := pgxmock.NewPool()
	if err != nil {
		t.Fatalf("pgxmock.NewPool: %v", err)
	}
	defer mock.Close()
	ctx := context.Background()
	mock.ExpectBegin() // outer
	mock.ExpectBegin() // fast-path savepoint
	ebN := mock.ExpectBatch()
	ebN.ExpectExec(`INSERT INTO traffic_event_normalized`).WithArgs(anyArgs(10)...).WillReturnError(errors.New("constraint-failed"))
	mock.ExpectRollback() // ROLLBACK TO SAVEPOINT (fast path)
	mock.ExpectBegin()    // per-row savepoint
	mock.ExpectExec(`INSERT INTO traffic_event_normalized`).WithArgs(anyArgs(10)...).WillReturnError(errors.New("constraint-failed"))
	mock.ExpectRollback().WillReturnError(errors.New("rollback rejected")) // per-row rollback warn branch
	...
	if err == nil || !strings.Contains(err.Error(), "insert normalized row") {
		t.Errorf("got err=%v, want wrapped 'insert normalized row'", err)
	}
	if err := mock.ExpectationsWereMet(); err != nil {
		t.Errorf("expectations: %v", err)
	}
...
// The fast-path savepoint cannot even be opened (outer tx unusable) → surfaces
// as a "begin normalized savepoint" error.
func TestTrafficWriter_InsertNormalizedPayloads_SavepointBeginFailure(t *testing.T) {
	mock, err := pgxmock.NewPool()
	if err != nil {
		t.Fatalf("pgxmock.NewPool: %v", err)
	}
	...
	ctx := context.Background()
	mock.ExpectBegin()                                            // outer
	mock.ExpectBegin().WillReturnError(errors.New("tx unusable")) // savepoint open fails

	tx, _ := mock.Begin(ctx)
	defer tx.Rollback(ctx) //nolint:errcheck
	w := NewTrafficEventWriter(nil, nil, TrafficEventWriterConfig{}, discardLogger(), nil)
	items := []pendingTrafficMessage{
		{event: TrafficEventMessage{ID: "x", RequestNormalizeStatus: "ok"}},
	}
	err = w.insertNormalizedPayloads(ctx, tx, items)
	if err == nil || !strings.Contains(err.Error(), "begin normalized savepoint") {
		t.Errorf("got err=%v, want wrapped 'begin normalized savepoint'", err)
	}
}

// Fast-path batch succeeds but RELEASE SAVEPOINT (savepoint Commit) fails →
// surfaces as a "release normalized savepoint" error; the raw rows are not
// rolled.
func TestTrafficWriter_InsertNormalizedPayloads_SavepointReleaseFailure(t *testing.T) {
	mock, err := pgxmock.NewPool()
	if err != nil {
		t.Fatalf("pgxmock.NewPool: %v", err)
	}
	...
	ebN := mock.ExpectBatch()
	ebN.ExpectExec(`INSERT INTO traffic_event_normalized`).WithArgs(anyArgs(10)...).WillReturnResult(pgconn.NewCommandTag("INSERT 0 1"))
	mock.ExpectCommit().WillReturnError(errors.New("release rejected")) // RELEASE SAVEPOINT fails
	...
	err = w.insertNormalizedPayloads(ctx, tx, items)
	if err == nil || !strings.Contains(err.Error(), "release normalized savepoint") {
		t.Errorf("got err=%v, want wrapped 'release normalized savepoint'", err)
	}
}

// Fast-path batch fails AND the fast-path savepoint Rollback fails (warn
// branch), then the per-row retry's savepoint cannot be opened → surfaces as a
// "begin normalized savepoint" error. Covers the fast rollback-warn branch and
// the per-row begin-failure branch in one flow.
func TestTrafficWriter_InsertNormalizedPayloads_FastRollbackWarnThenPerRowBeginFailure(t *testing.T) {
	mock, err := pgxmock.NewPool()
	if err != nil {
		t.Fatalf("pgxmock.NewPool: %v", err)
	}
	...
	ebN := mock.ExpectBatch()
	ebN.ExpectExec(`INSERT INTO traffic_event_normalized`).WithArgs(anyArgs(10)...).WillReturnError(errors.New("constraint-failed"))
	mock.ExpectRollback().WillReturnError(errors.New("rollback rejected")) // fast rollback warn
	mock.ExpectBegin().WillReturnError(errors.New("tx unusable"))          // per-row savepoint open fails
...
// Per-row RELEASE SAVEPOINT failure: fast batch fails → per-row exec succeeds →
// per-row Commit (release) fails → "release normalized savepoint <id>".
func TestTrafficWriter_InsertNormalizedPayloads_PerRowReleaseFailure(t *testing.T) {
	mock, err := pgxmock.NewPool()
	if err != nil {
		t.Fatalf("pgxmock.NewPool: %v", err)
	}
	...
	ebN := mock.ExpectBatch()
	ebN.ExpectExec(`INSERT INTO traffic_event_normalized`).WithArgs(anyArgs(10)...).WillReturnError(errors.New("constraint-failed"))
	mock.ExpectRollback() // fast rollback
	mock.ExpectBegin()    // per-row savepoint
	mock.ExpectExec(`INSERT INTO traffic_event_normalized`).WithArgs(anyArgs(10)...).WillReturnResult(pgconn.NewCommandTag("INSERT 0 1"))
	mock.ExpectCommit().WillReturnError(errors.New("release rejected")) // per-row release fails
...
// A 22P05 (jsonb null-character) sidecar row is recognised as a poison pill.
// The fast-path batch fails (poison row aborts it); on the row-by-row retry the
// poison row's savepoint is rolled back and the row is SKIPPED, while the good
// row that follows still commits — so the function returns nil and the rest of
// the batch (and the raw rows) proceed.
func TestTrafficWriter_InsertNormalizedPayloads_PoisonRowSkipped(t *testing.T) {
	mock, err := pgxmock.NewPool()
	if err != nil {
		t.Fatalf("pgxmock.NewPool: %v", err)
	}
	...
	ctx := context.Background()
	mock.ExpectBegin() // outer
	// Fast path: SAVEPOINT → batch fails on the poison row → ROLLBACK TO SAVEPOINT.
	// The batch has 2 queued rows, so the ExpectBatch needs 2 ExpectExec (the
	// first triggers the early return; pgx's deferred br.Close drains the rest).
	mock.ExpectBegin()
	ebN := mock.ExpectBatch()
	ebN.ExpectExec(`INSERT INTO traffic_event_normalized`).WithArgs(anyArgs(10)...).
		WillReturnError(&pgconn.PgError{Code: "22P05", Message: "unsupported Unicode escape sequence"})
	ebN.ExpectExec(`INSERT INTO traffic_event_normalized`).WithArgs(anyArgs(10)...).
		WillReturnError(&pgconn.PgError{Code: "22P05", Message: "unsupported Unicode escape sequence"})
	mock.ExpectRollback()
	// Row-by-row retry: poison row → savepoint Begin → Exec(22P05) → ROLLBACK, skipped.
	mock.ExpectBegin()
	mock.ExpectExec(`INSERT INTO traffic_event_normalized`).WithArgs(anyArgs(10)...).
		WillReturnError(&pgconn.PgError{Code: "22P05", Message: "unsupported Unicode escape sequence"})
	mock.ExpectRollback()
	// Good row → savepoint Begin → Exec ok → RELEASE.
	mock.ExpectBegin()
	mock.ExpectExec(`INSERT INTO traffic_event_normalized`).WithArgs(anyArgs(10)...).WillReturnResult(pgconn.NewCommandTag("INSERT 0 1"))
	mock.ExpectCommit()
	...
	w := NewTrafficEventWriter(nil, nil, TrafficEventWriterConfig{}, discardLogger(), nil)
	items := []pendingTrafficMessage{
		{event: TrafficEventMessage{ID: "poison", RequestNormalized: json.RawMessage(`{"x":"y"}`), NormalizeVersion: "1"}},
		{event: TrafficEventMessage{ID: "good", RequestNormalized: json.RawMessage(`{"a":"b"}`), NormalizeVersion: "1"}},
	}
	if err := w.insertNormalizedPayloads(ctx, tx, items); err != nil {
		t.Fatalf("got err=%v, want nil (poison row must be skipped, batch proceeds)", err)
	}
	if err := mock.ExpectationsWereMet(); err != nil {
		t.Errorf("expectations: %v", err)
	}
...
func TestTrafficWriter_InsertNormalizedPayloads_StatusOnlyTriggersInsert(t *testing.T) {
	...
	ebN := mock.ExpectBatch()
	ebN.ExpectExec(`INSERT INTO traffic_event_normalized`).WithArgs(anyArgs(10)...).WillReturnResult(pgconn.NewCommandTag("INSERT 0 1"))
	mock.ExpectCommit() // release savepoint
